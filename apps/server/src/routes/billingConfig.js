...
function normalizeProviderPolicy(value) {
  ...
  const channels = Array.isArray(parsed.channels) ? parsed.channels.filter((item) => item && item.id).slice(0, 30).map((item) => ({ id: String(item.id).slice(0,64), name: String(item.name || item.id).slice(0,120), provider: GENERATION_PROVIDER_IDS.has(String(item.provider || '').toLowerCase()) ? String(item.provider).toLowerCase() : 'custom', model: String(item.model || '').slice(0,200), endpoint: String(item.endpoint || '').slice(0,500), protocol: ['CHAT','RESPONSES','ANTHROPIC'].includes(String(item.protocol || '').toUpperCase()) ? String(item.protocol).toUpperCase() : 'CHAT', modelMappings: Array.isArray(item.modelMappings) ? item.modelMappings.filter((m) => m && (m.id || m.model)).slice(0,500).map((m) => ({ id: String(m.id || m.model).slice(0,200), displayName: String(m.displayName || m.id || m.model).slice(0,120), contextWindow: Number(m.contextWindow || 0) || null })) : [], modalities: [] })) : [];
...
export function normalizeAiProviderPolicy(raw) {
...
export async function handleAdminBillingConfig(ctx) {
  ...
  if (part === '/billing-config/ai-provider' && method === 'PUT') {
    ...
    const endpoint = body.endpoint === undefined ? before.endpoint : String(body.endpoint || '').trim();
    const displayName = body.displayName === undefined ? before.displayName : String(body.displayName || '').trim();
    const note = body.note === undefined ? before.note : String(body.note || '').trim().slice(0, 500);
    const websiteUrl = body.websiteUrl === undefined ? before.websiteUrl : String(body.websiteUrl || '').trim().slice(0, 500);
