...
import {
  ...
  row,
  rows,
  transaction, aq, arow, atransaction,
} from '../../lib.js';
...
export async function handleOrgCommunication(ctx) {
  ...
  if (part === '/inbox' && method === 'GET') {
    ...
    const items = await notificationRecipientRows(currentOrgId, auth.user.id);
  ...
  if (part === '/inbox' && method === 'POST') {
    ...
    await atransaction(async () => {
      await aq('INSERT INTO notifications(id,scope_type,org_id,sender_id,title,body,kind,target_url,audience,status,publish_at,pinned,created_at,updated_at) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?)', [noticeId, 'ORG', currentOrgId, auth.user.id, title, body, 'NOTICE', ctx.body?.targetUrl ? String(ctx.body.targetUrl).trim().slice(0, 500) : null, json(audience), 'PUBLISHED', now, bool(ctx.body?.pinned) ? 1 : 0, now, now]);
      await notificationRecipients(noticeId, 'ORG', currentOrgId, audience);
    });
    await audit(ctx, 'ORG_NOTIFICATION_CREATE', 'NOTIFICATION', noticeId, null, { roles });
    return normalizeNotification(await arow('SELECT * FROM notifications WHERE id=?', [noticeId]));
  }
  let match = part.match(/^\/inbox\/([^/]+)\/read$/);
  if (match && method === 'PUT') return await markNotificationRead(ctx, currentOrgId, match[1], auth.user.id);
  if (part === '/inbox/read-all' && method === 'PUT') return await markAllNotificationsRead(ctx, currentOrgId, auth.user.id);
  if (part === '/materials' && method === 'GET') {
    const result = await materialRows({ currentOrgId });
    return { items: result.items, total: result.total };
  }
  match = part.match(/^\/materials\/([^/]+)\/events$/);
  if (match && method === 'POST') {
    const material = await arow(`SELECT material.* FROM promo_materials material WHERE material.id=? AND material.status='ACTIVE' AND (material.visibility='ALL_ORGS' OR EXISTS (SELECT 1 FROM promo_material_assignments assignment WHERE assignment.material_id=material.id AND assignment.org_id=?))`, [match[1], currentOrgId]);
    ...
    await aq('INSERT INTO promo_material_events(id,material_id,org_id,user_id,event_type,created_at) VALUES (?,?,?,?,?,?)', [eventId, material.id, currentOrgId, auth.user.id, eventType, nowIso()]);
    await audit(ctx, 'PROMO_MATERIAL_' + eventType, 'PROMO_MATERIAL', material.id);
  ...
  if (part === '/help-feedback' && method === 'GET') {
    ...
    const items = await helpFeedbackRows(where + " ORDER BY CASE feedback.status WHEN 'SUBMITTED' THEN 0 WHEN 'IN_PROGRESS' THEN 1 ELSE 2 END, feedback.submitted_at DESC LIMIT 200", params);
    return {
      items,
      total: items.length,
  ...
  if (helpFeedbackMatch && method === 'GET') {
    if (auth.user.role !== 'ORG_ADMIN') throw errors.forbidden('仅机构管理员可以处理问题反馈', 'HELP_FEEDBACK_PERMISSION_DENIED');
    const feedback = (await helpFeedbackRows('feedback.id=? AND feedback.org_id=?', [helpFeedbackMatch[1], currentOrgId]))[0];
    if (!feedback) throw errors.notFound('反馈不存在', 'HELP_FEEDBACK_NOT_FOUND');
    return feedback;
  }
  if (helpFeedbackMatch && method === 'PUT') {
    if (auth.user.role !== 'ORG_ADMIN') throw errors.forbidden('仅机构管理员可以处理问题反馈', 'HELP_FEEDBACK_PERMISSION_DENIED');
    const feedbackRow = await arow('SELECT * FROM help_feedback WHERE id=? AND org_id=?', [helpFeedbackMatch[1], currentOrgId]);
    ...
    const now = nowIso();
    await aq('UPDATE help_feedback SET status=?,handled_by=?,handled_at=?,resolution=?,updated_at=? WHERE id=? AND org_id=?', [status, auth.user.id, now, resolution, now, feedbackRow.id, currentOrgId]);
    await audit(ctx, 'ORG_HELP_FEEDBACK_UPDATE', 'HELP_FEEDBACK', feedbackRow.id, normalizeHelpFeedback(feedbackRow), { status, resolution }, { orgId: currentOrgId });
    return (await helpFeedbackRows('feedback.id=? AND feedback.org_id=?', [feedbackRow.id, currentOrgId]))[0];
  }
  return null;
}
