...
export async function handleOrg(ctx) {
  ...
  if (classMatch && method === 'POST') {
    ...
    const cap = ctx.body?.sessionCreditCap === undefined || ctx.body?.sessionCreditCap === null ? null : integer(ctx.body.sessionCreditCap, '课堂积分上限'); const capability = ctx.body?.capabilities || {}; const sessionId = id('csession'); const now = nowIso(); const sessionKind = classMatch[2] === 'makeup' || ctx.body?.sessionKind === 'MAKEUP' ? 'MAKEUP' : 'REGULAR'; const deliveryMode = String(ctx.body?.deliveryMode || 'CANVAS').trim().toUpperCase(); if (!['CANVAS', 'VIBECODING'].includes(deliveryMode)) throw errors.badRequest('课堂入口类型无效', 'INVALID_DELIVERY_MODE');
    transaction(() => { q('INSERT INTO class_sessions(id,class_id,lesson_id,status,session_kind,delivery_mode,session_credit_cap,consumed_credits_total,ai_paused,student_call_cap,allow_text,allow_image,allow_music,allow_video,allow_podcast,allow_dubbing,started_by,started_at) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)', [sessionId, cls.id, lessonId, 'ACTIVE', sessionKind, deliveryMode, cap, 0, capability.aiPaused ? 1 : 0, capability.studentCallCap === undefined || capability.studentCallCap === null ? null : integer(capability.studentCallCap, '单学生调用次数', { min: 1, max: 100000 }), capability.allowText === undefined ? 1 : (capability.allowText ? 1 : 0), capability.allowImage === undefined ? 1 : (capability.allowImage ? 1 : 0), capability.allowMusic === undefined ? 1 : (capability.allowMusic ? 1 : 0), capability.allowVideo ? 1 : 0, capability.allowPodcast ? 1 : 0, capability.allowDubbing ? 1 : 0, auth.user.id, now]); q('UPDATE classes SET current_session_id=?,updated_at=? WHERE id=? AND org_id=?', [sessionId, now, cls.id, currentOrgId]); });
  ...
  // 兼容旧入口：学生积分接口改由 website handler 处理
  if (false && part === '/website/my-credits/summary' && method === 'GET') {
  ...
  if (part === '/website/my-credits/usage' && method === 'GET') {
    ...
    // 计算余额（逐条累减）
    const user = row('SELECT ai_credit_limit, ai_credits_used FROM users WHERE id = ?', [userId]);
    let currentBalance = Number(user.ai_credit_limit || 0) - Number(user.ai_credits_used);
