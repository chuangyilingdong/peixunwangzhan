...
import {
  ...
  assignmentActiveSql, PLATFORM_ADMIN_PERMISSIONS, platformPermissionForPathname, q, requirePlatformPermission, requireRole, row, rows, transaction, verifyPassword, arow, arows,
} from '../../lib.js';
...
export async function handleAudit(ctx, part, method) {
  if (part === '/audit-logs' && method === 'GET') {
    ...
    const sort = 'created';
    const total = Number((await arow('SELECT COUNT(*) n FROM audit_logs WHERE ' + q.where.replace(/audit\./g, ''), q.params))?.n || 0);
    const items = (await arows(auditListQuery(q.where) + ' LIMIT ? OFFSET ?', [...q.params, limit, (page - 1) * limit])).map(auditRow);
  ...
  if (part === '/audit-logs/summary' && method === 'GET') {
    ...
    const byAction = (await arows('SELECT action, COUNT(*) n FROM (' + base + ') s GROUP BY action ORDER BY n DESC LIMIT 20', q.params)).map((i) => ({ action: i.action, count: Number(i.n) }));
    const byActor = (await arows("SELECT actor_id, COALESCE(actor_name, actor_login, '系统') as actor_name, COUNT(*) n FROM (" + base + ') s GROUP BY actor_id, actor_name ORDER BY n DESC LIMIT 10', q.params)).map((i) => ({ actorId: i.actor_id || null, actorName: i.actor_name, count: Number(i.n) }));
    const byOrg = (await arows('SELECT org_id, org_name, COUNT(*) n FROM (' + base + ') s GROUP BY org_id, org_name ORDER BY n DESC LIMIT 10', q.params)).map((i) => ({ orgId: i.org_id || null, orgName: i.org_name || '平台', count: Number(i.n) }));
    const total = await arow('SELECT COUNT(*) n FROM audit_logs WHERE ' + q.where.replace(/audit\./g, ''), q.params);
  ...
  if (part === '/audit-logs/export' && method === 'GET') {
    ...
    const items = (await arows(auditListQuery(q.where) + ' LIMIT ' + limit, q.params)).map(auditRow);
  ...
  if (part === '/audit-logs/actions' && method === 'GET') {
    requireRole(ctx, ['SUPER_ADMIN']);
    const items = (await arows('SELECT action, COUNT(*) n FROM audit_logs GROUP BY action ORDER BY action ASC')).map((i) => ({ action: i.action, count: Number(i.n) }));
