...
export async function handleAdmins(ctx, part, method) {
  if (part === '/platform-admins' && method === 'GET') {
    ...
    const adminUsers = await arows('SELECT user.* FROM users user WHERE ' + where + ' ORDER BY ' + sortSql + ' LIMIT ? OFFSET ?', [...params, limit, (page - 1) * limit]);
    const meta = await userLoginMeta(adminUsers.map((item) => item.id));
    const mfaRows = adminUsers.length
      ? await arows(`SELECT user_id, status FROM user_mfa_credentials WHERE user_id IN (${adminUsers.map(() => '?').join(',')})`, adminUsers.map((item) => item.id))
      : [];
  ...
  if (part === '/platform-admins' && method === 'POST') {
    ...
    await assertDisplayNameAvailable(displayName, { role: 'SUPER_ADMIN' });
    ...
    await aq('INSERT INTO users(id,org_id,login,display_name,role,permissions,password_hash,status,created_at,updated_at) VALUES (?,?,?,?,?,?,?,?,?,?)', [adminId, null, login, displayName, 'SUPER_ADMIN', json(permissions), hashPassword(password), body.status === 'DISABLED' ? 'DISABLED' : 'ACTIVE', now, now]);
    await audit(ctx, 'PLATFORM_ADMIN_CREATE', 'USER', adminId, null, { login, permissions });
    return normalizeUser(await arow('SELECT * FROM users WHERE id=?', [adminId]), { includeAuthMeta: true });
  }
  const adminLogMatch = part.match(/^\/platform-admins\/([^/]+)\/audit-logs$/);
  if (adminLogMatch && method === 'GET') {
    requireRole(ctx, ['SUPER_ADMIN']);
    const target = await arow("SELECT * FROM users WHERE id=? AND role='SUPER_ADMIN' AND deleted_at IS NULL", [adminLogMatch[1]]);
    ...
    const items = (await arows('SELECT audit.*, target_user.display_name target_name FROM audit_logs audit LEFT JOIN users target_user ON target_user.id=audit.target_id AND audit.target_type=\'USER\' WHERE audit.actor_id=? ORDER BY audit.created_at DESC LIMIT ' + limit, [target.id])).map((item) => ({
  ...
  if (adminMatch && method === 'PUT') {
    const auth = requireRole(ctx, ['SUPER_ADMIN']); const target = await arow("SELECT * FROM users WHERE id=? AND role='SUPER_ADMIN' AND deleted_at IS NULL", [adminMatch[1]]);
    if (!target) throw errors.notFound('平台管理员不存在', 'ADMIN_NOT_FOUND');
    const body = ctx.body || {};
    if (body.login !== undefined && String(body.login).trim() !== target.login && await arow('SELECT id FROM users WHERE login=?', [String(body.login).trim()])) throw errors.conflict('登录名已存在', 'LOGIN_EXISTS');
    ...
    if (displayName !== target.display_name) await assertDisplayNameAvailable(displayName, { role: 'SUPER_ADMIN', excludeUserId: target.id });
    ...
    if (target.status === 'ACTIVE' && !hasAnyPlatformPermission(permissions)) {
      const effectiveAdmins = await arows("SELECT id FROM users WHERE role='SUPER_ADMIN' AND status='ACTIVE' AND deleted_at IS NULL");
    ...
    if (body.status !== undefined) {
      ...
      if (status === 'DISABLED' && target.status !== 'DISABLED') await lastSuperAdminGuard(target);
    }
    ...
    await aq('UPDATE users SET login=?,display_name=?,permissions=?,password_hash=?,status=?,updated_at=? WHERE id=?', [login, displayName, json([...new Set(permissions)]), passwordHash, status, nowIso(), target.id]);
    if ((status === 'DISABLED' && target.status !== 'DISABLED') || body.password !== undefined) await aq('UPDATE sessions SET superseded_at=? WHERE user_id=? AND superseded_at IS NULL', [nowIso(), target.id]);
    await audit(ctx, 'PLATFORM_ADMIN_UPDATE', 'USER', target.id, { login: target.login, displayName: target.display_name, status: target.status }, { displayName, status, passwordChanged: body.password !== undefined, permissions });
    return normalizeUser(await arow('SELECT * FROM users WHERE id=?', [target.id]), { includeAuthMeta: true });
  }
