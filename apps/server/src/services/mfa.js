...
export async function mfaCredential(userId) {
  return await arow('SELECT * FROM user_mfa_credentials WHERE user_id=?', [userId]) || null;
}

export async function mfaSummary(userId) {
  const record = await mfaCredential(userId);
...
export async function mfaEnabledFor(userId) {
  const record = await mfaCredential(userId);
  return !!record && record.status === 'ENABLED';
}

export async function startMfaSetup(userId, { account, issuer }) {
  const record = await mfaCredential(userId);
...
export async function enableMfa(userId, code) {
  ...
  return { recoveryCodes: plainCodes, ...await mfaSummary(userId) };
}
...
export async function regenerateRecoveryCodes(userId, code) {
  ...
  const plainCodes = generateRecoveryCodes();
  const stored = plainCodes.map((item) => ({ hash: hashRecoveryCode(item), usedAt: null, createdAt: nowIso() }));
  await aq('UPDATE user_mfa_credentials SET recovery_codes=?,updated_at=? WHERE user_id=?', [json(stored), nowIso(), userId]);
  return { recoveryCodes: plainCodes, ...await mfaSummary(userId) };
}
...
export async function disableMfa(userId, code) {
  ...
  const result = await verifyMfaChallenge(userId, code);
  if (!result.ok) throw errors.badRequest(result.reason === 'MISSING' ? '请输入动态验证码或恢复码' : '验证码不正确', 'MFA_INVALID_CODE');
  await aq('DELETE FROM user_mfa_credentials WHERE user_id=?', [userId]);
  return await mfaSummary(userId);
}
...
export async function verifyMfaChallenge(userId, code) {
  ...
  if (matched) {
    await aq('UPDATE user_mfa_credentials SET last_totp_counter=?,updated_at=? WHERE user_id=?', [matched.counter, nowIso(), userId]);
  ...
  for (let index = 0; index < codes.length; index += 1) {
    ...
    await aq('UPDATE user_mfa_credentials SET recovery_codes=?,updated_at=? WHERE user_id=?', [json(codes), nowIso(), userId]);
