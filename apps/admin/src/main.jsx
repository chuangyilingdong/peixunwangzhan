...
function AdminInbox({ api }) {
  ...
  const 家机构 = useData(() => api.get('admin/家机构'), [api]);
  ...
  return <>
    ...
    <div className="split"><Panel title="新建通知"><form onSubmit={create}>
      ...
      {form.scope === 'ORG_IDS' ? <label>指定机构<select multiple value={form.orgIds} onChange={(e) => setForm({ ...form, orgIds: [...e.target.selectedOptions].map((option) => option.value) })}>{家机构.data?.items?.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select></label> : null}
    ...
    <Panel title="平台通知记录">{inbox.loading || 家机构.loading ? <Loading /> : inbox.error ? <ErrorState error={inbox.error} onRetry={inbox.refresh} /> : inbox.data?.items?.length ? <>
      ...
      <ListResultSummary total={inbox.data.total} page={inbox.data.page} totalPages={inbox.data.totalPages} label="条通知" /><div className="table-wrap"><table><thead><tr><th>通知</th><th>范围</th><th>投递 / 未读</th><th>状态</th><th>发布时间</th><th>操作</th></tr></thead><tbody>{inbox.data.items.map((item) => <tr key={item.id}><td><strong>{item.pinned ? '置顶 · ' : ''}{item.title}</strong><div className="muted">{item.kind} · {item.body}</div></td><td>{item.audience?.scope === 'ALL_ORGS' ? '全部用户' : `${item.audience?.orgIds?.length || 0} 家机构`}<div className="muted">{item.audience?.roles?.join(' / ')}</div></td><td>{item.recipientCount} / {item.unreadCount}</td><td><状态 value={item.status} /></td><td>{item.publishAt ? formatDate(item.publishAt) : '—'}</td><td><div className="row-actions">{['DRAFT', 'SCHEDULED', 'RECALLED'].includes(item.status) ? <button className="secondary-button" onClick={() => update(item, 'PUBLISHED')}>立即发布</button> : null}{item.status === 'PUBLISHED' ? <button className="text-button" onClick={() => update(item, 'RECALLED')}>撤回</button> : null}</div></td></tr>)}</tbody></table></div><Pagination page={inbox.data.page} totalPages={inbox.data.totalPages} onChange={setPage} disabled={inbox.loading} /></> : <><div className="filters"><input value={filters.search} placeholder="搜索通知标题或内容" onChange={(e) => { setFilters({ ...filters, search: e.target.value }); setPage(1); }} /><select value={filters.status} onChange={(e) => { setFilters({ ...filters, status: e.target.value }); setPage(1); }}><option value="">全部状态</option><option value="DRAFT">草稿</option><option value="SCHEDULED">已排期</option><option value="PUBLISHED">已发布</option><option value="RECALLED">已撤回</option></select></div><Empty title="No 条通知 match" body="可以调整关键词或状态筛选条件。" /></>}</Panel>
  </>;
}

function Admin物料s({ api }) {
  const [filters, setFilters] = useState({ search: '', status: '', category: '', visibility: '' });
  const [page, setPage] = useState(1); const [limit, setLimit] = useState(20); const [sort, setSort] = useState('created');
  const query = useMemo(() => { const params = new URLSearchParams(Object.entries(filters).filter(([, value]) => value)); params.set('page', String(page)); params.set('limit', String(limit)); params.set('sort', sort); return params; }, [filters, page, limit, sort]);
  const 个物料 = useData(() => api.get(`admin/个物料?${query.toString()}`), [api, query]); const 家机构 = useData(() => api.get('admin/家机构'), [api]);
  const [form, setForm] = useState({ title: '', description: '', category: 'GENERAL', visibility: 'ALL_ORGS', orgIds: [], mimeType: '', resourceUrl: '', coverUrl: '' });
  const [message, setMessage] = useState(''); const [saving, setSaving] = useState(false); const [stats, set统计] = useState({ loading: false, data: null, error: null });
  async function create(event) {
    event.preventDefault(); setSaving(true); setMessage('');
    try { await api.post('admin/个物料', { ...form, orgIds: form.visibility === 'ALL_ORGS' ? [] : form.orgIds }); setForm({ title: '', description: '', category: 'GENERAL', visibility: 'ALL_ORGS', orgIds: [], mimeType: '', resourceUrl: '', coverUrl: '' }); setMessage('物料元数据已保存。'); 个物料.refresh(); } catch (err) { setMessage(err.message); } finally { setSaving(false); }
  }
  async function toggle(item) { try { await api.put(`admin/个物料/${item.id}`, { status: item.status === 'ACTIVE' ? 'DISABLED' : 'ACTIVE' }); 个物料.refresh(); } catch (err) { setMessage(err.message); } }
  async function load统计(item) { set统计({ loading: true, data: null, error: null }); try { set统计({ loading: false, data: await api.get(`admin/个物料/${item.id}/stats`), error: null }); } catch (error) { set统计({ loading: false, data: null, error }); } }
  return <>
    <PageHeader eyebrow="平台内容" title="素材与宣传物料" description="维护招生海报、课程介绍和活动资料的元数据、授权范围与真实使用统计。" actions={<button className="secondary-button" onClick={个物料.refresh}>刷新</button>} />
    ...
    <div className="split"><Panel title="新增宣传物料"><form onSubmit={create}>
      ...
      {form.visibility === 'ASSIGNED_ORGS' ? <label>指定机构<select multiple value={form.orgIds} onChange={(e) => setForm({ ...form, orgIds: [...e.target.selectedOptions].map((option) => option.value) })}>{家机构.data?.items?.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select></label> : null}
