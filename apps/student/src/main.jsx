...
function StudentPage({ kind, api }) {
  const pages = {
    courses: ['我的课程', '查看机构为你开通的课程与课时，按老师安排进入对应项目创作。', ['课程包', '平台课程按机构与班级统一发放'], ['学习进度', '上课记录会成为你的课程进度']],
    credits: ['AI / 魔法石', '了解你的本周期可用额度、课堂权限和 AI 创作使用说明。', ['个人额度', '按套餐与课堂规则使用 AI 创作能力'], ['魔法石', '完成创作和课堂活动可以积累个人成就']],
    account: ['个人账号', '查看你的机构归属、账号状态和创作权限。', ['账号状态', '启用、有效期和班级归属由机构统一管理'], ['安全设置', '支持后续接入修改密码与个人信息']],
    help: ['帮助与下载', '查看客户端安装、课堂登录和创作常见问题。', ['客户端下载', '支持 macOS Apple 芯片与 Windows 64 位'], ['创作指南', '学会用中文描述、预览、修改并发布作品']],
  };
  const [title, description, cards] = pages[kind];
  return <><PageHeader eyebrow="AI魔法学院 · 小小创作者" title={title} description={description} actions={kind === 'help' ? <a className="primary-button" href="http://localhost:5176/download">下载客户端 ↗</a> : null} /><div className="metrics">{cards.map((item, index) => <MetricCard key={item[0]} label={item[0]} value={index ? '准备中' : '可查看'} hint={item[1]} tone={index ? 'teal' : 'violet'} />)}</div><Panel title={kind === 'help' ? '创作小提示' : '使用说明'}><Notice tone="info">{kind === 'courses' ? '老师开启课堂后，你可以从“我的项目”继续完成本节课的创作。画布编辑器会在项目内保持原样，后续统一优化。' : '这部分页面已按 AI魔法学院学生端的账户与学习结构建设；需要套餐、有效期与用量明细的真实数据接口会在后端完成后接入。'}</Notice></Panel></>;
}
