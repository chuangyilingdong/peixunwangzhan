...
function StudentTasks({ api }) {
  ...
  async function update(task, action) { try { await api.post(`student/learning/tasks/${task.id}/${action}`, {}); setMessage(action === 'submit' ? '任务已提交，等待老师点评。' : '已开始任务。'); tasks.refresh(); } catch (error) { setMessage(error.message); } }
