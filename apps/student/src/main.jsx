...
function CanvasWorkspace({ api }) {
  ...
  return <main className="student-canvas-shell">
    <header className="student-canvas-topbar">
      ...
      <div className="student-canvas-actions"><button className="ghost-canvas-button" onClick={() => navigate('/classroom')}>课程大厅</button><button className="primary-canvas-button" disabled={busy || !changed} onClick={save}>{busy ? '保存中…' : '保存并退出'}</button></div>
    </header>
    ...
    <div className="student-canvas-submitbar"><div><strong>完成作品后记得提交</strong><span>老师会根据你的画布内容进行点评</span></div><div className="student-submit-actions"><button className="secondary-button" onClick={() => navigate('/classroom')}>退出课堂</button><button className="primary-canvas-button student-submit-button" disabled={!editable || busy || !draft || !hasNodes} onClick={async () => { if (!editable || !draft) return; setBusy(true); try { await api.post(`student/projects/${project.data.id}/submit`, { canvasSnapshot: draft, description: `完成${lessonTitle}作品` }); setMessage('作品已提交，老师可以看到你的课堂作品了。'); project.refresh(); } catch (err) { setMessage(err.message); } finally { setBusy(false); } }}>{busy ? '提交中…' : '提交作品 ✨'}</button></div></div>
