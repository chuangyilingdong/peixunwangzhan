...
const appBase = process.env.VITE_APP_BASE || '/student/';
export default defineConfig({
  root,
  base: appBase,
  plugins: [react()],
  resolve: { alias: [{ find: '@platform/shared', replacement: shared }, { find: '@platform/canvas', replacement: canvas }] },
  server: { port: 5174, strictPort: true, proxy: { '/api': 'http://localhost:8787' } },
  preview: { port: 6174, strictPort: true, proxy: { '/api': 'http://localhost:8787' } },
