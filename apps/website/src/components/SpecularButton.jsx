// 来源：React Bits 的 SpecularButton（JavaScript + CSS 版），按用户要求原样搬入。
// https://reactbits.dev/ —— 原样保留以便日后跟上游 diff；只加这一行来源说明。
//
// 平台侧的注意事项（改它之前先读）：
// · 依赖 `ogl`（工作区根 package.json 里已装）；WebGL 绘制，**不加载任何外部资源**，
//   所以与生产 CSP（script-src 'self'）不冲突。
// · 它在 useEffect 里才创建 WebGL 上下文，SSR 渲染期不碰 document —— 三端页面渲染守卫
//   （scripts/p70）会真渲染首页，**别把 Renderer 的创建挪到渲染期**，那会让守卫整片红。
// · 每个按钮一个 WebGL 上下文；浏览器上限约 16 个，首页只有这两个按钮，安全。
...
const SpecularButton = ({
  ...
  useEffect(() => {
    ...
    // 平台补充（上游代码没有这条）：系统设了「减少动态效果」就**不创建 WebGL** ——
    // 不只是不显示：连 rAF 循环与 GPU 上下文都不起。按钮退化成普通玻璃胶囊（底色/描边/文字照常）。
