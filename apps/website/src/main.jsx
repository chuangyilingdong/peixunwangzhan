...
function ScrubVideo({ src, progress, variant = 'hero', poster }) {
  ...
  return <div className={'ic-video-wrap ic-video-' + variant}><div className="ic-video-fallback" aria-hidden="true"><span /><i>✦</i><b>{variant === 'hero' ? 'AI MAGIC CIRCLE' : 'MAKE / PLAY / SHARE'}</b></div>{poster && <img className="ic-video-poster" src={poster} alt="" aria-hidden="true" />}{<video ref={videoRef} className={'ic-scrub-video ' + (status === 'ready' ? 'is-ready' : '')} src={src} poster={poster} playsInline muted preload="metadata" aria-hidden="true" />}</div>;
}
