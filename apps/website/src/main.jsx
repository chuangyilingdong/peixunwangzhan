...
function ScrubVideo({ src, progress, variant = 'hero' }) {
  ...
  return <div className={'ic-video-wrap ic-video-' + variant}><div className="ic-video-fallback" aria-hidden="true"><span /><i>✦</i><b>{variant === 'hero' ? 'AI MAGIC CIRCLE' : 'MAKE / PLAY / SHARE'}</b></div><video ref={videoRef} className={'ic-scrub-video ' + (status === 'ready' ? 'is-ready' : '')} src={src} playsInline muted preload="metadata" aria-hidden="true" /></div>;
}
