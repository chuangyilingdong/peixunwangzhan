...
function Classes({ api, user }) {
  ...
  async function start(classId, makeup = false) {
    ...
    try { await api.post(`org/classes/${classId}/sessions/${makeup ? 'makeup' : 'start'}`, { lessonId, sessionKind: makeup ? 'MAKEUP' : 'REGULAR', deliveryMode: sessionModes[classId] || 'CANVAS', capabilities: { allowImage: true, allowMusic: true } }); setMessage(makeup ? '补课课堂已开始。' : '课堂已开始。'); await classes.refresh(); await loadDetail(classId, true); }
