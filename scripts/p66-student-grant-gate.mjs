...
try {
  ...
  check('① 未许可课包不出现在学生课程列表', !seriesBefore, JSON.stringify(seriesBefore || {}).slice(0, 240));
  ...
  check('① 未许可课包不出现在课程中心',
    dashboardBefore.status === 200 && !courseBefore,
  ...
  const teacher2 = (await api('/api/auth/login', { method: 'POST', body: { login: 'teacher-2', password: 'teach123' } })).data.token;
  assert.ok(teacher2, '第二位教师登录失败');
  const occupied = await api('/api/org/sessions', { method: 'POST', token: teacher, body: { lessonId: seeded.lessonId } });
  check('教师已有未结束课堂时拒绝重复创建', occupied.status === 409 && occupied.error?.code === 'TEACHER_SESSION_OCCUPIED');
  const emptySession = await api('/api/org/sessions', { method: 'POST', token: teacher2, body: { lessonId: seeded.lessonId, title: 'P66 空课堂' } });
  const emptyStart = await api(`/api/org/sessions/${emptySession.data.id}/start`, { method: 'POST', token: teacher2 });
  ...
  assert.equal((await api(`/api/org/sessions/${emptySession.data.id}/dissolve`, { method: 'POST', token: teacher2, body: {} })).status, 200);
  ...
  const otherSession = await api('/api/org/sessions', { method: 'POST', token: teacher2, body: { lessonId: seeded.lessonId, title: 'P66 同期另一个课堂' } });
  const conflictAdd = await api(`/api/org/sessions/${otherSession.data.id}/students`, { method: 'POST', token: teacher2, body: { studentIds: [seeded.studentId] } });
  ...
  const candidates = await api(`/api/org/sessions/${otherSession.data.id}/candidates`, { token: teacher2 });
  ...
  const dissolved = await api(`/api/org/sessions/${otherSession.data.id}/dissolve`, { method: 'POST', token: teacher2, body: { reason: 'P66 测试解散' } });
