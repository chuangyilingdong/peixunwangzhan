...
const sends = (sessionId, studentId) => Number(readRow(
  'SELECT vibecoding_sends FROM session_students WHERE session_id=? AND student_id=?', [sessionId, studentId],
)?.vibecoding_sends || 0);
const usageCount = () => Number(readRow('SELECT COUNT(*) AS n FROM usage_records')?.n || 0);
...
try {
  ...
  await check('数到了 1 次（只数学生自己打的）', () => assert.equal(sends(sessionId, student.id), 1));
  ...
  await check('dsh 注入的 <system-reminder> 块**不算**学生发送（本轮的真回归）', async () => {
    ...
    assert.equal(sends(sessionId, student.id), 2, '注入块被算成发送了（一次发送会吃掉好几次额度）');
  });
  await check('注入块排在最后也一样不算（dsh 的新鲜上下文是后追加的）', async () => {
    ...
    assert.equal(sends(sessionId, student.id), 2);
  });
  ...
  await check('运行时上下文快照（不带方括号包裹）也不算（第二个注入源）', async () => {
    ...
    assert.equal(sends(sessionId, student.id), 2, '运行时上下文快照被算成发送了');
  });
  await check('跟着 dsh 真实顺序走一遍：学生消息 + 两个注入块 = 只算 1 次', async () => {
    ...
    assert.equal(sends(sessionId, student.id), 2, '一次发送被多记了（注入块没排干净）');
  });
  ...
  await check('⭐ 被拦下的这一次**不新增 usage_records**（门禁在上游之前，既不花算力也不进账）', () => {
    assert.equal(usageCount(), usageBefore, '超限的请求打到了上游（会白花平台的钱）');
  });
  ...
  await check('被拦下的那一次推进「观察到的最大值」、但不改变「对外报的已用」（两个口径分开）', async () => {
    assert.equal(sends(sessionId, student.id), 3, '观察到的最大值要落库（含被拦下的那几次，否则压缩历史就能漏放）');
