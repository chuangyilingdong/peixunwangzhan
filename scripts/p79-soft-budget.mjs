...
try {
  ...
  await aq("INSERT INTO organizations(id,name,contract_start_at,contract_expires_at,created_at,updated_at) VALUES ('org-soft','Soft','2026-01-01','2030-01-01','2026-01-01','2026-01-01')");
  await aq("INSERT INTO users(id,org_id,login,display_name,role,password_hash,created_at,updated_at) VALUES ('student-soft','org-soft','soft','Soft','STUDENT','x','2026-01-01','2026-01-01')");
  await aq("INSERT INTO class_sessions(id,org_id,status,platform_budget_fen,created_at) VALUES ('soft-session','org-soft','ACTIVE',50,'2026-01-01')");
  let calls = 0;
  ...
  for (let n = 0; n < 2; n++) {
    const budget = await classroomBudgetStatus('soft-session');
    ...
    await recordAiUsage({orgId:'forged-org',userId:'student-soft',sessionId:'soft-session',modality:'TEXT',model:'soft-model',status:'SUCCESS',costFen:999999,pricing:{compute:provider.compute}});
  }
  assert.equal(calls,2, 'second call must reach upstream after 60 exceeds budget 50');
  assert.equal((await classroomBudgetStatus('soft-session')).knownCostFen,120);
  assert.equal((await classroomBudgetStatus('soft-session')).budgetState,'OVER_BUDGET');
  assert.ok((await arows('SELECT * FROM usage_records')).every(r=>r.org_id==='org-soft' && r.cost_fen===0 && r.credits_charged===0));
  assert.ok((await arows('SELECT * FROM compute_attempts')).every(r=>r.org_id==='org-soft' && r.class_session_id==='soft-session'));
  await aq("UPDATE class_sessions SET platform_budget_fen=500 WHERE id='soft-session'");
  ...
  const status = await classroomBudgetStatus('soft-session');
  ...
  assert.ok(!JSON.stringify(await arows('SELECT * FROM compute_attempts')).includes('private-only'));
  ...
  await saveComputeGatewayConfig({enabled:true,baseUrl:'https://gateway.test'},{password:'admin-password'});
  ...
  assert.ok(Array.isArray(await lessonPlatformBudgetOverview()));
