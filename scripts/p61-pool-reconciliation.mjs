...
const seeded = await (async () => {
  ...
  const students = await arows("SELECT id, org_id FROM users WHERE role='STUDENT' LIMIT 2");
  const [a, b] = students;
  
  const now = new Date().toISOString();
  await aq(`INSERT INTO usage_records(
      id,org_id,user_id,modality,model,credits_charged,status,fail_code,pricing_snapshot,cost_fen,series_id,created_at
    ) VALUES (?,?,?,?,?,?,?,?,?,?,?,?)`, ['r1', a.org_id, a.id, 'TEXT', 'm', 1, 'SUCCESS', null, '{}', 60, lesson.series_id, now]);
  await aq(`INSERT INTO usage_records(
      id,org_id,user_id,modality,model,credits_charged,status,fail_code,pricing_snapshot,cost_fen,series_id,created_at
    ) VALUES (?,?,?,?,?,?,?,?,?,?,?,?)`, ['r2', a.org_id, a.id, 'VIDEO', 'm', 1, 'SUCCESS', null, '{}', 500, lesson.series_id, now]);
  await aq(`INSERT INTO usage_records(
      id,org_id,user_id,modality,model,credits_charged,status,fail_code,pricing_snapshot,cost_fen,series_id,created_at
    ) VALUES (?,?,?,?,?,?,?,?,?,?,?,?)`, ['r3', b.org_id, b.id, 'TEXT', 'm', 0, 'FAILED', 'X', '{}', 0, lesson.series_id, now]);
