/**
 * P78 学生进课的「学员许可」门禁（2026-09-13，用户确认按梳理文档 4.3「叠加」口径）。
 *
 * 口径：学生要能上某节课，**两条都得满足** ——
...
const temp = fs.mkdtempSync(path.join(os.tmpdir(), 'p78-grant-gate-'));
...
const grantNow = async () => {
  ...
  else await aq(`INSERT INTO student_course_grants(id,org_id,student_id,series_id,granted_at) VALUES (?,?,?,?,datetime('now'))`, ['p78_grant', seeded.orgId, seeded.studentId, seeded.seriesId]);
  
};
const revokeNow = async () => {
   
  await aq("UPDATE student_course_grants SET revoked_at=datetime('now'),revoke_reason='P78 守卫：平台兜底撤销' WHERE student_id=? AND series_id=?", [seeded.studentId, seeded.seriesId]);
...
try {
  ...
  const createProject = () => api('/api/student/projects', { method: 'POST', token: student, body: { courseLessonId: seeded.lessonId, title: 'P78 门禁' } });
  const createConversation = () => api('/api/student/vibecoding/conversations', { method: 'POST', token: student, body: { lessonId: seeded.lessonId, title: 'P78 门禁会话' } });
  ...
  check('① 课程列表已把该课包标成未授权（items[].hasGrant === false）', !seriesBefore, JSON.stringify(seriesBefore || {}).slice(0, 240));
  ...
  const admin = (await api('/api/auth/login', { method: 'POST', body: { login: 'org-admin', password: 'org123' } })).data.token;
  ...
  const created = await api('/api/org/sessions', { method: 'POST', token: teacher, body: { lessonId: seeded.lessonId, title: 'P78 门禁课堂' } });
  check('②b 老师能创建课堂（待上课）', created.status === 200 && created.data?.status === 'PENDING', JSON.stringify(created).slice(0, 240));
  const sessionId = created.data?.id;
  check('教师已有待上课课堂不能再创建', (await api('/api/org/sessions', { method: 'POST', token: teacher, body: { lessonId: seeded.lessonId } })).error?.code === 'TEACHER_SESSION_OCCUPIED');
  ...
  const emptySession = await api('/api/org/sessions', { method: 'POST', token: admin, body: { lessonId: seeded.lessonId, title: 'P78 空课堂' } });
  const emptyStart = await api(`/api/org/sessions/${emptySession.data.id}/start`, { method: 'POST', token: admin });
  ...
  check('同课堂重进幂等', (await createProject()).data.id === okProject.data.id);
  check('上课中不可移除', (await api(`/api/org/sessions/${sessionId}/students/${seeded.studentId}`, { method: 'DELETE', token: teacher })).status === 409);
  // 同一节课上「未结束的参与」不能同时被两个课堂占用（用户规则 3.4/3.5）：
  // 此刻他正在画布课堂里，试着把他加到另一个课堂 → 必须被拒，并说清占用的课堂
  const otherSession = await api('/api/org/sessions', { method: 'POST', token: admin, body: { lessonId: seeded.lessonId, title: 'P78 同期另一个课堂' } });
  const conflictAdd = await api(`/api/org/sessions/${otherSession.data.id}/students`, { method: 'POST', token: admin, body: { studentIds: [seeded.studentId] } });
  ...
  const candidates = await api(`/api/org/sessions/${otherSession.data.id}/candidates`, { token: admin });
  ...
  const vibeSession = await api('/api/org/sessions', { method: 'POST', token: teacher, body: { lessonId: seeded.lessonId, title: 'P78 VibeCoding 课堂', deliveryMode: 'VIBECODING' } });
  ...
  check('Vibe同课堂重进幂等', (await createConversation()).data.id === okConversation.data.id);
  // 2026-09-16 口径：入口按课时已发布的类型放行（p78 的夹具课时是 ["CANVAS","VIBECODING"]），
  ...
  const next = await api('/api/org/sessions', { method: 'POST', token: teacher, body: { lessonId: seeded.lessonId, deliveryMode: 'CANVAS' } });
  await api(`/api/org/sessions/${next.data.id}/students`, { method: 'POST', token: teacher, body: { studentIds: [seeded.studentId] } });
  await api(`/api/org/sessions/${next.data.id}/start`, { method: 'POST', token: teacher });
  const nextProject = await createProject();
  check('新课堂不复用旧创作', nextProject.data.id !== okProject.data.id);
  const legacyUpdate = await api(`/api/student/projects/${okProject.data.id}`, { method: 'PUT', token: student, body: { title: '不应修改旧课堂' } });
  ...
  const writeLegacyRoster = async (targetSessionId) => {
    
    
    const row = await arow('SELECT id, org_id, lesson_id, series_id FROM class_sessions WHERE id=?', [targetSessionId]);
    ...
    await aq("INSERT INTO session_students(id,session_id,student_id,org_id,lesson_id,series_id,status,added_by,added_at,updated_at) " +
      "VALUES (?,?,?,?,?,?, 'PENDING', NULL, ?, ?)", [`sess_stu_legacy_${Math.random().toString(36).slice(2, 8)}`, row.id, seeded.studentId, row.org_id, row.lesson_id, row.series_id, now, now]);
  ...
  await writeLegacyRoster(busySession.data.id);
  ...
  await writeLegacyRoster(legacySession.data.id);
     ...
     （客户端据此写「查看作品」）、以及 `session-state` 能把"老师已结束"告诉学生（老师结束 → 学生退出画布）。 */
  {
    ...
    await aq("UPDATE session_students SET status='INCOMPLETE' WHERE student_id=? AND status IN ('PENDING','ACTIVE')", [seeded.studentId]);
    await aq("UPDATE class_sessions SET status='ENDED', ended_at=datetime('now') WHERE status IN ('PENDING','ACTIVE')");
