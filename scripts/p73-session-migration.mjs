...
let migratedSnapshot;
{
  ...
  const teacher = await arow("SELECT id FROM users WHERE login='teacher-1'");
  const students = await arows("SELECT id FROM users WHERE role='STUDENT' ORDER BY login LIMIT 2");
  const lesson = await arow("SELECT id, series_id, title FROM course_lessons WHERE status='PUBLISHED' ORDER BY sort LIMIT 1");
  ...
  await aq("CREATE UNIQUE INDEX idx_class_sessions_active ON class_sessions(class_id) WHERE status = 'ACTIVE'");
  await aq(`INSERT INTO class_sessions(id,class_id,lesson_id,status,delivery_mode,started_by,started_at)
    VALUES ('p73_live',?,?, 'ACTIVE','CANVAS',?,?)`, [cls.id, lesson.id, teacher.id, '2026-09-01T10:00:00.000Z']);
  await aq(`INSERT INTO class_sessions(id,class_id,lesson_id,status,delivery_mode,allow_video,started_by,started_at,ended_by,ended_at,ended_reason)
    VALUES ('p73_done',?,?, 'ENDED','VIBECODING',0,?,?,?,?, 'MANUAL')`, [cls.id, lesson.id, teacher.id, '2026-09-02T10:00:00.000Z', teacher.id, '2026-09-02T11:00:00.000Z']);
  // 已扩展过的旧库也必须保留非默认课堂类型，不能重建后重置为 REGULAR。
  await aq("ALTER TABLE class_sessions ADD COLUMN session_kind TEXT NOT NULL DEFAULT 'REGULAR'");
  await aq("UPDATE class_sessions SET session_kind='TRIAL' WHERE id='p73_done'");
  fixture.legacyRows = await arows('SELECT * FROM class_sessions ORDER BY id');
  // 两名学员都有成功使用证据，金额分别为 100 和 0，均应完课。
  await aq(`INSERT INTO usage_records(id,org_id,user_id,class_session_id,modality,model,credits_charged,status,cost_fen,created_at)
    VALUES ('p73_u1',?,?,'p73_done','TEXT','m',0,'SUCCESS',100,'2026-09-02T10:30:00.000Z')`, [orgId, students[0].id]);
  await aq(`INSERT INTO usage_records(id,org_id,user_id,class_session_id,modality,model,credits_charged,status,cost_fen,created_at)
    VALUES ('p73_u2',?,?,'p73_done','TEXT','m',0,'SUCCESS',0,'2026-09-02T10:31:00.000Z')`, [orgId, students[1].id]);
  fixture.usageRows = await arows('SELECT * FROM usage_records ORDER BY id');
  const ddl = (await arow("SELECT sql FROM sqlite_master WHERE name='class_sessions'")).sql;
...

{
  ...
  check('迁移后：class_id 可空、started_* 可空、新列齐全',
    ...
    ddl.slice(0, 140));
  const rows = await arows("SELECT * FROM class_sessions WHERE id LIKE 'p73_%' ORDER BY id");
  ...
  assert.deepEqual(await arows('SELECT * FROM usage_records ORDER BY id'), fixture.usageRows, '首次迁移必须保留全部用量字段');
  ...
  check('迁移后：用量记录没被动（软引用还在）', Number((await arow("SELECT COUNT(*) n FROM usage_records WHERE id LIKE 'p73_%'")).n) === 2);
  check('迁移后：旧的部分唯一索引（一班一活跃课堂）已移除', !(await arows("SELECT name FROM sqlite_master WHERE type='index' AND name='idx_class_sessions_active'")).length);

  const students = await arows("SELECT * FROM session_students WHERE session_id='p73_done'");
  ...
  check('进行中的老课堂没有被回填（ACTIVE 不算完课）', Number((await arow("SELECT COUNT(*) n FROM session_students WHERE session_id='p73_live'")).n) === 0);
  ...
  try {
    await aq("INSERT INTO class_sessions(id,title,org_id,series_id,lesson_id,teacher_id,status,delivery_mode,created_at,updated_at) VALUES ('p73_pending','新课堂',?,?,?,?,'PENDING','CANVAS',datetime('now'),datetime('now'))", [fixture.orgId, fixture.seriesId, fixture.lessonId, fixture.teacherId]);
    await aq("INSERT INTO session_students(id,session_id,student_id,org_id,lesson_id,series_id,status,added_at) VALUES ('p73_ss','p73_pending',?,?,?,?,'PENDING',datetime('now'))", [fixture.studentB, fixture.orgId, fixture.lessonId, fixture.seriesId]);
