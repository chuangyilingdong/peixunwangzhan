...
try {
  ...
  await aq("INSERT INTO course_lessons(id,series_id,title,sort,status,delivery_mode,classroom_config,canvas_template_snapshot,created_at,updated_at) VALUES ('lesson1','series1','测试课时',1,'PUBLISHED','CANVAS',?,?,?,?)", [JSON.stringify({ version: 3 }), '{}', now, now]);
  await aq("INSERT INTO course_lesson_capabilities(lesson_id,capability,created_at) VALUES ('lesson1','image',?)", [now]);
  ...
  await aq("INSERT INTO course_lesson_materials(id,group_id,title,description,material_type,asset_url,snapshot,sort,created_at,updated_at) VALUES ('box-image-1','mg1','素材1','','GENERATION_BOX',NULL,?,1,?,?)", [JSON.stringify({ box: { modality: 'IMAGE', model: '', aspectRatio: '16:9', resolution: '1k' }, content: '' }), now, now]);
  ...
  await aq("INSERT INTO billing_packages(id,org_id,name,allow_image,status,created_at,updated_at) VALUES ('pkg1','org1','套餐',1,'ACTIVE',?,?)", [now, now]);
  await aq("INSERT INTO generation_jobs(id,org_id,user_id,project_id,modality,provider,model,prompt,status,credits_charged,created_at,box_id) VALUES ('job1','org1','stu1','proj1','IMAGE','local-mock','canvas-mock-v1','测试','SUCCEEDED',1,?,'box-image-1')", [now]);
  await aq("INSERT INTO media_assets(id,job_id,org_id,user_id,project_id,modality,label,asset_url,created_at) VALUES ('asset1','job1','org1','stu1','proj1','IMAGE','素材','mock://asset',?)", [now]);

  // 独立夹具：真实教师只开一个课堂，项目明确绑定该课堂。
  await aq("INSERT INTO users(id,org_id,login,display_name,role,password_hash,status,created_at,updated_at) VALUES ('teacher1','org1','teacher1','教师','TEACHER','x','ACTIVE',?,?)", [now, now]);
  await aq("INSERT INTO class_sessions(id,title,org_id,series_id,lesson_id,teacher_id,status,delivery_mode,allow_text,allow_image,started_by,started_at,created_at,updated_at) VALUES ('session1','测试课堂','org1','series1','lesson1','teacher1','ACTIVE','CANVAS',1,1,'teacher1',?,?,?)", [now, now, now]);
  await aq("INSERT INTO session_students(id,session_id,student_id,org_id,lesson_id,series_id,status,added_by,added_at) VALUES ('ss1','session1','stu1','org1','lesson1','series1','ACTIVE','teacher1',?)", [now]);
  await aq("UPDATE student_projects SET class_session_id='session1' WHERE id='proj1'");
  check((await arow("SELECT COUNT(*) n FROM class_sessions WHERE teacher_id='teacher1' AND status IN ('PENDING','ACTIVE')")).n === 1, '教师夹具应只有一个未结束课堂');
