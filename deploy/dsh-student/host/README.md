...
> **现状（2026-09-16，真机已验证）**：暂时就用**生产机自己**当宿主 —— 上面装好了 docker 29.1.3、
> 导入了 `dsh-student:local`（2.6GB），`smoke-test.sh` 在真机上 **8/8 全过**：容器起得来、
> 入口闸门 403/200、容器里带图调用打通生产网关（`https://iicili.cyou/api/gateway/v1`），
> 那一通真的落进了生产账本（`deepseek-flash / SUCCESS / 带图=true`）。
> 用户口径：机器先这么用，后续再扩。**实测容量**：平台进程只占 35MB，可用内存约 1.25GB，
> 一个学生容器空转约 407MB → **这台机器同时跑 2 个学生容器就到底了**（10 人同时上课必须换机器）。
