...
const SHIMS = [
  {
    ...
    //
    // 渲染条件是 WelcomeNotice 里那句：状态是 idle/loading 或者 acknowledged 为 true 就返回 null。
    // 原本靠预置 `DSH_HOME/settings.yaml` 里的 `ui-onboarding.welcomeNoticeVersion` 让服务端认定「已读」，
    // 但在**同机独立用户**这条路上实测不生效：设置作用域在那边不是持久化的，客户端会走
    // "unavailable"/内存那条分支把已读置 false（容器模式同一份文件却有效）。与其继续追设置链路，
    // 这里直接让这个组件**永不渲染**。
    // 什么时候能删：等 dsh 把这个公告从客户端拆出去，或我们把设置作用域的问题查清。
    //
    // ⚠️ 补丁必须**穿透 pnpm 的符号链接**改到真身（这里用 writeFileSync，它会跟随符号链接）。
    // 在真机上用 `sed -i` 改这个路径会把符号链接替换成普通文件，而打包器读的是 `.pnpm` 里的真身
    // —— 结果就是「补丁明明写进去了、线上却还是老代码」（2026-09-16 在这上面耗了很久）。
    file: path.join(DSH_MODULES, 'dsh-client-ui-settings-models/lib/client.js'),
    from: 'function WelcomeNotice(props) {',
    to: 'function WelcomeNotice(props) { return null;',
    why: '学生端不显示 dsh 的开发内测公告（它会挡住界面）',
  },
  {
