#!/usr/bin/env node
/**
 * ⚠️ 已被取代，**不要再跑**（2026-09-18 标注）：本脚本把官网文案里的「魔法石」写成「灵动值」，
 * 而 2026-09-18 起「灵动值」这个营销口径已被整体删除（平台侧只保留「授权次数」）。
 * 重跑它会**把灵动值写回生产内容**，与当前的 migrate-website-content-20260918.mjs 正好相反。
 * 它当初做过的事已经落库，这里只作为历史记录保留（本仓库惯例：删代码不删记录）。
 *
