  ...
  return <main className="student-canvas-shell">
    ...
    <section className="student-canvas-layout">
      <aside className={`student-tool-rail ${toolPanel ? 'is-open' : ''}`}>
        ...
        <button className={`student-tool-button ${toolPanel === 'generate' ? 'is-active' : ''}`} type="button" onClick={() => setToolPanel((value) => value === 'generate' ? null : 'generate')}><span>✦</span><small>AI生成</small></button><button className={`student-tool-button ${toolPanel === 'capabilities' ? 'is-active' : ''}`} type="button" onClick={() => setToolPanel((value) => value === 'capabilities' ? null : 'capabilities')}><span>⚙</span><small>能力</small></button>
