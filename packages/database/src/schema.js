...
//
// ✅ **可以增量改造**（这一点我最初判断错了，实测后更正）：转换期这一层是"同步实现的异步外壳"
//    （底下就是同步的 row/rows/q，没有任何真实 I/O），而 `await` 一个**已 resolve** 的 Promise
//    只会进微任务队列 —— Node 会在处理下一个宏任务（别的请求）之前把微任务排空，
//    **所以不会发生"别的请求挤进同一个事务"**。于是可以一个文件一个文件地改、应用全程可用。
//
// ⚠️ 真正的交界在**切换那一刻**：换成 mysql2 之后 `await` 是一次真实网络往返，**会**让出事件循环，
//    那时事务必须用专用连接 —— 用 AsyncLocalStorage 记录"当前事务的连接"、
