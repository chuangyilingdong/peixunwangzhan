...
> 创建日期：2026-09-02  
> 当前阶段：**P9-D01 生产切换已完成并验证通过。`iicili.cyou` 作为线下机构使用的受控生产平台运行，不以公众开放注册、在线交易或内容社区为当前目标；内测数据继承为生产数据，不重新 seed。**旧站已按用户授权清除且无备份，不得宣称存在旧站回滚能力。全部 6 个历史种子账号均已禁用并作废旧会话；后续账号由平台负责人和机构管理员按组织层级创建。外部告警、值班电话、正式法律页、真实 AI、支付和消息通道均不属于当前线下交付前置项。**
...
- [x] 2026-09-06 追加修复已发布：提交 `6ea4f339133d391463c47457bfebfd5736e2125e`，production release `20260906T152014Z`。模型读取现在按 `channelId` 使用当前渠道密钥，并将 `/v1/models`、`/v1/videos`、`/v1/images/generations` 等 Endpoint 统一归一化为模型列表地址；服务健康与四端入口复核通过。
...
- [x] **P9-D01 生产部署与切换（2026-09-04）。**
  ...
  - 账号安全：全部 6 个种子账号（`root`、`org-admin`、`teacher-1`、`teacher-2`、`student-1`、`student-2`）已重置为随机强密码（保存于服务器 `/root/.ai-kids-platform-production-credentials`，权限 0600），全部旧会话作废；新密码登录 + `/api/me` 六角色通过，旧密码全部 401。
...
- [x] **P9-D02 生产备份、账号加固、最小告警与观察基线（2026-09-04）。**
  ...
  - 最小告警：升级每分钟 `monitoring-healthcheck.sh`，覆盖 API 失败、磁盘 ≥80%、证书 14 天窗口、备份超过 26 小时 / 失败四类；正常与故障注入路径均验证，状态写入 `production/state/last-alert-state.json`。按线下机构范围，不建设外部短信 / 飞书 / 邮件通知渠道。
...
- [~] **P9-D02 生产 24 小时运行观察：按用户决定停止，不作为当前上线阻塞（2026-09-05）。**
  - 原观察窗口为 2026-09-04 20:24:38 CST 至 2026-09-05 20:24:38 CST；保留 `docs/p9/P9-D02-production-24h-observation.md` 作为历史运维记录，不再等待窗口自然结束。
  - 已完成并保留的基线：生产健康检查、每日备份 / 恢复演练、账号加固、Nginx 敏感路径加固、四端公网入口与 API 冒烟。
  - 当前直接进入线下机构交付验收与上线前缺陷收口；不接入外部短信、邮件、飞书告警，不等待值班联系人或故障升级电话。
  - 当前业务边界：法律页、举报 / 申诉 / 内容审核 / 监护人流程不作为线下机构交付前置项；AI 保持 `local-mock`，真实支付 / 短信 / 邮件 / OSS / 微信未接入。
- [x] **线下机构真实交付闭环验收与上线发布准备（代码侧完成，生产切换待受控发布）。**
  - 流程：平台创建机构 → 机构管理员创建教师 / 学生 → 教师创建班级 → 教师加入学生 → 教师配置已授权课程 / 课时 → 教师开课 → 学生学习、创建并提交项目 → 教师查看并点评作品。
  - 本次收口：seed 教师账号不再携带 `MANAGE_CLASSES` / `MANAGE_MEMBERS`；班级编辑、归档、成员增删、课程 / 课时配置、开课、课堂控制和结束课堂统一按教师本人负责 / 获授权班级放行，不依赖额外权限码。
  - 验证：教师职责专项 9/9；P4-C01 27/27；P8-Q03 52/52；P8-Q04 54/54；P4-03 50/50；P8-Q01 11/11；P8-Q02 17/17；P8-Q06 13/13；四端生产构建通过；语法检查和 `git diff --check` 通过；全部使用临时 SQLite，未触碰默认库和生产数据。
  - 当前唯一剩余动作：待 GitHub Actions / 受控 ECS 发布窗口完成最新 commit 的 production release 切换；不把外部告警、值班电话、法律页或 24 小时观察列为阻塞。
### 2026-09-05 用户职责边界确认

- 当前产品是面向线下机构的受控教学平台，不以公众开放注册、在线交易或内容社区为交付前置条件。
- 平台负责人负责创建机构和机构管理员；机构管理员负责创建 / 导入教师与学生账号、账号状态和机构级经营事项。
- **教师负责创建班级、将本机构学生加入 / 移出自己负责或获授权的班级、为班级配置已授权课程 / 课时并开展课堂。**
- 教师不能访问其他机构数据，也不能操作其他教师未负责 / 未授权的班级；学生不能调用机构教务接口。
- 外部短信、邮件、飞书告警、值班电话、故障升级联系人、7×24 外部值守、正式法律页和对外合规宣传不纳入本阶段线下交付验收项。
...
- [x] **P4-01（缺口清单编号）：平台计费与作品闭环第一批**
  ...
  - 完成记录（2026-09-02）：
    ...
    - [ ] 风险与后续：平台下架当前由前端限制仅对 `PUBLISHED` 作品操作，服务端严格状态机校验留待“统一领域状态机”事项；平台接口仍为角色级 `SUPER_ADMIN`，已转入 P9-R03 逐域收紧；完成后按业务域权限码返回 403 `PERMISSION_DENIED`。
  - 边界：本批不修改 `packages/canvas`，不伪造支付到账、模型配置和运营数据，不部署线上环境。
- [x] **P4-02（缺口清单编号）：学生课程、额度与账号安全第一批**
  - 优先级：P0
  - 完成记录（2026-09-02）：
...
| P4 | 非画布真实业务 API 与页面闭环 | `[x]` | 核心机构交付、学生学习、AI 队列控制、通知物料、账务、成员生命周期与权限闭环已完成；真实外部 AI、OSS、支付、外部消息及正式合规按后续范围保留。 |
...
- [-] **P4-02 数据迁移与演示数据隔离规范（盘点与规范完成，生产处置待变更窗口）**
  - 优先级：P0
  - 实现范围：建立 schema 版本、迁移脚本、临时数据库测试方法、seed 数据说明和备份前置检查。
  - 验收：新环境可以从零初始化；升级旧库可以迁移；测试不污染主数据库；迁移失败可回滚或从备份恢复。
  - 完成记录（2026-09-04）：`docs/p4-02-demo-data-isolation.md` 定义 seed 锚点、基础教学资产保留、演示行为数据隔离、报表排除和生产处置前置条件；`scripts/p4-02-demo-data-inventory.mjs` 在临时 SQLite 初始化 + seed 后只读盘点通过。生产演示数据归档 / 清理未执行，待 24 小时观察结束后另行批准。
...
- [x] **P4-A05 作品库与公开作品基础闭环（举报 / 违规治理扩展 `[~]` 暂缓）** ✅ 2026-09-04
  - 优先级：P1
  - 页面：作品库。
  - 实现范围：跨机构作品筛选、审核发布、精选、下架、作品详情和展示权限；举报、申诉、违规处理和内容治理扩展按用户决策 `[~]` 暂缓。
  - 完成记录：平台作品库、详情、精选、下架和举报处理接口已就位；服务端 `PUT /api/admin/works/:id/unpublish` 通过统一状态机强制仅 `PUBLISHED → REJECTED`，重复下架返回 `INVALID_WORK_TRANSITION`，并写入非法转换审计。平台详情、精选和举报路径也有发布态校验。
  - 验收：`p4-01-state-machine.mjs` 临时 SQLite **28 pass / 0 fail**，覆盖发布后下架、重复下架拒绝和审计；P4-A05 专项已有 **43 pass / 0 fail**；未修改 `packages/canvas`，未触碰真实线上数据库。
  - 剩余（非暂缓）：公开分享展示权限 / 站外分发仍未实现；举报、申诉、违规处理及相关内容治理按用户决策继续 `[~]` 暂缓。
- [x] **P4-A06 计费与模型配置后台**
  - 优先级：P0
  - 页面：计费与模型。
...
- [x] **P4-O09 机构站内信、宣传物料与阿飞提醒**
  ...
  - 完成记录（2026-09-03，本批：自动提醒）：
    ...
    - [x] 验收脚本（2026-09-04）：新增 `scripts/p4-o09-reminders.mjs`，每次使用隔离临时 SQLite；低余额 / 合同到期扫描、重复扫描去重、通知与收件人落库、禁用用户不投递共 **9/9 通过**，证据 `artifacts/p4-o09-reminders.txt`。未打开生产数据库。
  - 剩余：忽略状态、失败重试与高可用队列（队列层已具备，外部通道未接入）、阿飞微信通道、真实上传与下载代理（依赖 P4-C04 深化 / P6 OSS）、课节 24h 提醒（需 class_sessions.start_at 字段）。
  - 2026-09-04 收口说明：本次仅完成隔离验收脚本与证据；P4-O09 总项仍保持 [-]，因为课节 24h 提醒、阿飞微信、真实文件代理和部分队列运营能力仍未交付。
...
- [-] **P6-A01 供应商适配器与密钥管理**
  ...
  - 当前状态：已完成不产生外部费用的 provider 契约、配置校验、稳定错误码映射和隔离测试；用户已确认允许学生内容外发；首个真实供应商、模型、endpoint、服务端密钥和具体预算仍由平台端自行填写，生产继续使用 `local-mock`。
  - 完成记录（2026-09-05）：
    - 状态：`[-]`
...
- 补充完成记录（2026-09-05，用户确认允许学生内容外发 + 平台 / 机构两级预算）：
    ...
    - 安全边界：策略层 `allowStudentExternalContent` 默认开启，平台超管可关闭，机构端只读；关闭时任意外部供应商策略下学生项目生成前置返回 `STUDENT_EXTERNAL_AI_BLOCKED`，不创建任务、不扣积分、不产生用量；开启后未配置真实 adapter / 服务端密钥只允许明确失败，不假成功。平台预算与机构预算均由各自端维护，`0` 表示不启用上限，非 0 值按单次和当日成功扣费累计校验。
...
7. **外部阻塞：等待重新决策。** 真实 AI、OSS、支付、微信 / 短信 / 邮件、品牌邮箱和正式公开上线，不在本轮伪造或强行接入。

---

## 9. P9｜部署、灰度、正式上线与运营交接

### 9.1 当前阶段：生产环境（2026-09-04 已切换）

P9-I01～P9-I06 已完成并保留为内测阶段记录。2026-09-04 用户确认 `iicili.cyou` 直接作为生产域名，生产切换已完成；`learning-platform-production` 在 `127.0.0.1:8789` 承载四端与 API，internal-test 停止并禁用但保留全部回滚资产。当前进入 24 小时观察、每日备份验证与生产收尾，不代表正式合规、真实 AI 或外部支付能力已经完成。

### 9.2 正式公开上线：环境与交付链路

- [x] **P9-D00 旧线上环境盘点、备份与新平台替换发布方案。** ✅ 2026-09-04
  - 优先级：P0
  - 目标环境（2026-09-04 更新）：`iicili.cyou`（39.106.183.200）；旧站 `/opt/learning-platform`、`learning-platform.service`、8787 与旧 Nginx 配置已按用户授权清除，当前唯一线上服务为新平台内测 release，API 仅监听 `127.0.0.1:8788`。
  ...
  - 验收：
    - [x] 用户已于 2026-09-04 明确免除旧站备份并授权直接清除；已按授权清除旧服务、旧目录与旧 Nginx 配置。
    - [x] 旧站已按用户授权直接清除；新平台内测继续保留 release、发布前数据库备份与回滚脚本。
    - [ ] Nginx `/api/` 代理、SPA 回退、静态资源缓存、HTTPS 和错误页在预发 / 演练中验证；
    - [ ] 防火墙 / 云安全组只允许 `80/TCP`、`443/TCP` 对公网访问，`8788/TCP` 仅绑定回环地址且没有公网放行规则；旧 `8787` 不得被新配置覆盖；
    - [x] 已按用户决策放弃旧站备份 / 旧站回滚；新平台继续保留 release 与内测数据库回滚能力。
    - [x] 已记录发布窗口、验证结果与边界：2026-09-04 20:10 CST 清理，四端入口 + API + 8787 关闭回归 6/6 通过；旧站回滚命令不存在，因为用户明确免备份并授权不可恢复清除。
  - 完成记录（2026-09-04）：只读盘点与方案见 `docs/p9/P9-D00-readonly-inventory-plan.md`，证据 `evidence/p9-d00-20260904/readonly-inventory-redacted.txt`。随后用户明确免除备份并授权直接清除：停止 / 禁用并删除 `learning-platform.service`，删除 `/opt/learning-platform` 与三份旧 Nginx 配置。清理后 Nginx 校验通过、8787 关闭、新平台 active，公网官网 / 三工作台 / API 健康 6/6 通过。旧站数据已不可恢复删除，不再存在旧站备份或旧站回滚能力。
- [x] **P9-D01 确定生产部署架构** ✅ 2026-09-04
  - 优先级：P0
  - 范围：服务器 / 容器、操作系统、进程管理、数据库形态、对象存储、CDN、网络、WAF、区域和成本。
  - 完成记录：ADR 已实施并验证，见 `docs/p9/P9-D01-production-architecture-ADR.md`：单 ECS + production / internal-test 双目录、systemd、SQLite WAL、Nginx 单域名入口、版本化 release 原子切换；公网仅 22/80/443，API 仅回环 8789。
  - 验收：架构与资产清单、访问控制、容量与扩容路径、备份策略已确定；每日 03:00 备份保留 14 天并完成恢复演练。联系人 / 值班 / 外部告警渠道仍为 ADR 遗留项，不宣称完成。
- [-] **P9-D02 建立开发、测试 / 预发、生产环境隔离（生产侧完成，完整隔离待收口）**
  - 优先级：P0
...
- [-] **P9-D03 CI/CD、GitHub Actions 与制品管理**
  ...
  - 本批完成记录（2026-09-04）：
    ...
    - [x] 本地等价验证：`node --check`、P4-01 `28/28`、P4-02 成功、P4-O09 `9/9`、`pnpm run build` 历史基线通过；本次未改 `packages/canvas`。    - [x] GitHub Actions 实际运行留证：run `33880904346`（commit `ecae5a8`）成功，依赖安装、语法检查、隔离验收、四端构建与四端制品上传全部通过；此前 run `33880647166` 的根 `dist` 路径故障已修复。
  - 验收：
    - [x] 从干净环境可重复构建并上传可定位制品：GitHub Actions run `33880904346` 成功；生产发布仍保持人工批准，服务器原子切换 / 自动失败回滚尚未接入。
    ...
    - [ ] 部署脚本使用版本化 release 目录或等效原子切换，避免直接覆盖运行中的版本；（生产侧已有 release 目录与 `rollback-production.sh`，本次补充停止服务前输入校验与异常时自动尝试恢复服务；Actions 尚未接入受控发布脚本）
...
- [x] **P9-R01 生产数据迁移与备份前检查** ✅ 2026-09-04
  - 优先级：P0
  - 完成记录：切换前完成 internal-test 数据库备份，并复制为 production 数据库；原内测库保留为切换前快照，未重新 seed。每日备份 `ai-kids-platform-production-daily-backup.timer` 03:00 Asia/Shanghai、保留 14 天；首次备份 SHA256 与 SQLite `integrity_check` 通过。
  - 恢复演练：隔离目录 + `127.0.0.1:18789` 恢复验证健康 200、数据计数一致，演练后释放端口；未覆盖生产库。后续仍需每月例行恢复演练和异机 / 对象存储副本。
- [~] **P9-R02 灰度 / 白名单 / Feature Flag 策略**
  - 优先级：P0
  - 验收：可按机构 / 用户开启新功能、真实 AI、支付或公开作品；异常时可无发布回滚地关闭总开关。
- [x] **P9-R03 上线前业务验收（UAT）** ✅ 2026-09-04
  - 优先级：P0
  - 完成记录：P8-Q07 已按平台、机构管理员、教师、学生、官网访客完成角色 UAT，未记录未闭环 P0 缺陷；生产切换后四端公网入口、`/api/health`、HTTPS 与安全头复测通过。
  - 边界：财务 / 法务签字留档未形成，客服与运营验收角色未单独留痕；此完成口径限于技术与角色 UAT，不包含正式合规结论。
- [x] **P9-R04 正式上线日操作清单** ✅ 2026-09-04
  - 优先级：P0
  - 完成记录：冻结窗口完成旧站授权清理、internal-test 停止与数据继承、production release `20260904T122323Z` 切换、Nginx 切换、四端与 API 冒烟、健康 timer / 备份 timer / 告警脚本上线；internal-test 与 Nginx 变更前资产保留。
  - 验收：操作记录见 ADR 第 9～11 节与 `docs/p9/P9-D02-production-24h-observation.md`。公告、客服排班和值班通讯录未作为本次上线日范围，后续纳入 P9-R06 交接包。
- [~] **P9-R05 上线后 7 / 30 天观察与复盘（按当前线下交付范围后置）**
  - 优先级：P0
  - 处理：不再把 24 小时 / 7 天 / 30 天观察作为当前线下机构交付阻塞；保留已有监控、备份和恢复能力，后续按真实机构规模另行复盘。
  - 边界：不建设外部通知、值班电话、客服工单或内容审核流程；当前 `local-mock` 不产生真实模型成本。
- [-] **P9-R06 运营与技术交接包**
  - 优先级：P0
  - 范围：部署文档、账户 / 权限台账、应急手册、客服 FAQ、运营 SOP、课程维护 SOP、财务对账 SOP、数据字典、接口文档。
  - 验收：非开发人员能按 SOP 完成日常操作；关键账号可由授权人员接管；离职 / 交接流程明确。
  - 本批完成记录（2026-09-04）：新增 `docs/p9/P9-R06-operations-handover.md`，覆盖生产资产、账号权限（不含凭据）、巡检、备份、发布 / 回滚、应急、课程维护、财务数据边界、数据字典和升级模板。
  - 遗留：值班联系人 / 外部通知渠道、P9-D04 Secret 台账、GitHub Actions 实际运行留证和非开发人员实操验收仍未完成，P9-R06 保持 `[-]`。
...
### 2026-09-05 P4 机构成员账号闭环与统一回归补证
- [x] 新增 `scripts/p4-o15-ai-generation-cancel.mjs`，验证排队任务取消后状态为 `FAILED`、错误码 `GENERATION_CANCELLED`、worker 释放、积分不扣且不写成功用量记录；本地与生产源码隔离执行均通过。

- [x] 新增 `scripts/p4-o14-org-member-account-loop.mjs`，隔离临时 SQLite 验证机构管理员创建学生、教师越权拒绝、停用、旧密码失效、密码重置、重新启用、班级归属维护、批量导入预览与提交，**10/10** 通过。
- [x] 修复 `p8-q04-e2e.mjs` 对官网首页启动占位层的误报，官网导航与四端登录链路回归 **54/54** 通过。
- [x] 统一回归证据：P8 质量 11/11、单元 17/17、API 52/52、安全 32/32、会话 27/27、租户隔离 20/20；P10 上传 9/9、访问矩阵 6/6。
- [x] commit `228744f` 已推送；生产源码已拉取并执行 P4-O14 **10/10**；生产服务监听 `127.0.0.1:8789`，内部 health 正常，公网四端 HTTP 验收 **4/4**。
...
| 日期 | 变更 | 状态 / 验证 |
|---|---|---|
| 2026-09-04 | 回滚脚本安全补强：停止 production 前校验目标 release / 服务入口 / 数据库备份输入；切换中断时尝试恢复服务。 | 静态 shell 结构复核与 `git diff --check` 通过；未在真实生产执行，未触碰生产数据库或 internal-test 资产。 |`r`n| 2026-09-04 | GitHub Actions 首次运行诊断与修复：发现制品上传路径误指向根 `dist`，修正 CI / production gate 为上传四端 `apps/*/dist`。 | run `33880647166` 前置检查与构建通过但上传失败；修复后 run `33880904346` 全部成功，P9-D03 的 CI 验证子项收口，生产部署自动化仍未接入。 |`r`n| 2026-09-04 | 生产公网敏感路径复核与 Nginx 规则补强准备：新增只读 smoke、补齐生产配置模板 / RUNBOOK，并确认 `/server.js` 等仍被 SPA fallback 返回 200。 | 本地 Node 语法与 diff 检查通过；公网 `/api/health=200`、安全头存在；P9-D05 保持 `[-]`，等待授权 ECS 终端执行 Nginx 备份、`nginx -t`、reload 与复测。 |`r`n| 2026-09-04 | 生产切换与运维补强完成：P9-D01 架构实施、每日备份与恢复演练、真实管理员与种子账号禁用、最小告警、Nginx 扫描面加固；同步修正 P9 清单状态。 | `learning-platform-production` active/enabled；四端公网 200；备份 `20260904T124238Z` SHA256 / integrity 通过；恢复演练健康 200；种子账号 DISABLED 且会话清零；告警正常与故障注入验证通过；24 小时观察截止 2026-09-05 20:24 CST。 |
...
- [x] 用户确认：外部短信 / 邮件 / 飞书告警、值班联系人、故障升级电话、正式法律 / 合规展示不属于当前线下机构交付范围，不作为阻塞项。
...
> 请先读取 `D:\学习平台\platform-v2\docs\AI少儿编程平台-完整上线执行总控.md`、`docs\p9\P9-D01-production-architecture-ADR.md` 与 `docs\p9\P9-D02-production-24h-observation.md`，以“当前唯一下一步”和未勾选事项为准推进。当前生产为 `https://iicili.cyou`（`learning-platform-production`，8789 仅回环，release `20260904T122323Z`），优先事项是：24 小时观察收口、每日 03:00 备份首次执行验证、P4-02 演示数据隔离规范、P4-A05 服务端作品状态机、P9-D03 最小 CI/CD 与 P9-R06 交接包。AI 仍是 `AI_PROVIDER=local-mock`，法律页仍是准备稿，不得宣传真实 AI、正式合规、支付 / 短信 / 邮件 / OSS / 微信能力；生产发布必须人工批准。每次完成事项必须更新总控勾选、完成记录、验证方式和变更日志；继续禁止修改 `D:\学习平台\platform-v2\packages\canvas`、删除 internal-test 回滚资产、输出密码密钥或 `.env` 内容、直接写入生产数据库。
...
### 2026-09-04 P4-02 演示数据隔离收口

- [x] 完成临时 SQLite seed 盘点：6 个 seed 用户、1 个示例机构、1 个课程系列、5 个课时、1 个班级、3 个班级成员可稳定复现；行为数据初始为 0。
...
- [~] P9-D02 24 小时观察按用户决定于 **2026-09-05 13:55 CST** 主动停止；未宣称 24 小时观察完成。外部告警通知和值班升级不属于当前范围；正式 Secret 管理器仍作为规模扩大或接入真实第三方服务前的后续事项。
...
- [x] 产品定位确认：`iicili.cyou` 是面向线下机构的受控生产平台，不以面向公众的注册、交易或内容社区为当前目标。
- [x] 账号模型确认：平台负责人创建机构和机构管理员；机构管理员创建或导入教师、学生，负责班级、课程和成员授权；学生和教师不走公开自助注册。
- [x] 当前不纳入范围：外部短信 / 邮件 / 飞书告警、值班电话和外部升级联系人。现有 systemd 健康检查、备份状态和服务器日志作为内部运维基线；发生异常由平台负责人或授权运维人员处理。
- [x] 当前不纳入范围：面向公众的正式法律页、公开合规宣传、真实支付、短信、邮件、OSS、微信和真实 AI 供应商接入。法律页可保留为准备稿，但不作为本平台线下交付前置条件。
- [ ] 下一阶段重点：执行线下机构首批受控交付；使用固定清单复核机构管理员创建教师 / 学生、分班授权、禁用 / 重置密码、教师授课和学生学习作品闭环。
- [ ] 延后事项：Feature Flag、异机 / 对象存储备份、正式 Secret 管理器和更复杂的多机构运营报表，只有在真实机构规模或接入外部能力前再立项。

### 2026-09-05 生产真实业务闭环验收

- [x] 使用固定生产检测账号完成登录与身份确认：平台、机构、教师、学生 4 个角色均 HTTP 200，`/api/me` 角色匹配。
- [x] 教师职责闭环通过：教师读取机构课程和学生名册，创建临时班级，将 `student-1` 加入班级，为班级分配课时，并成功开启课堂。
- [x] 学生学习闭环通过：学生读取 dashboard 和课程，在教师开启的课堂中创建项目、保存第二版本、确认版权并提交作品。
- [x] 教师教学反馈闭环通过：教师在本班待批作品列表中看到学生作品，审核为 `APPROVED` 并写入点评；学生端可读取审核结果和教师点评。
- [x] 验收收口：教师结束课堂，临时验收班级已归档；本次验收没有留下进行中的课堂。
- [x] 本次共 24 项 API / 业务检查全部通过（登录、权限、课程、班级、成员、课堂、项目、提交、审核、点评）。
- [x] 业务验收后数据库备份：`/srv/ai-kids-platform/production/backups/20260905T051314Z/platform.db`；备份对应当前 release `/srv/ai-kids-platform/production/releases/20260905T045348Z`。
- [i] 作品项目 `project_7e658e1d358245748595` 与作品 `work_cc4c9548957d4328b014` 作为已审核历史验收数据保留；按产品状态机，已提交 / 已审核项目不能直接归档，不回写生产库规避历史关联。
- [ ] 下一步进入真实机构交付前的功能补齐：机构管理员账号创建 / 导入教师学生、禁用与密码重置、教师班级与课程管理的页面级回归；不重新扩大外部告警、公开注册、法律页范围。

### 2026-09-05 账号生命周期修复发布

- [x] 真实验收中发现：账号先停用后直接清除会被用户状态机拒绝，无法满足机构清理无用账号的操作路径。
- [x] 已修复机构成员删除接口：对 `DISABLED` 成员允许幂等进入软删除；历史业务关联和审计数据仍保留，不物理删除。
- [x] 本地教师职责回归 9/9 通过；生产发布 commit `04fd077`，release `20260905T051622Z`。
- [x] 生产回归：创建临时学生 → 停用 → 直接删除 → 名册不可见，HTTP 200 全部通过；临时账号已清理。
- [x] 发布后服务 active，健康检查、四端入口和敏感路径安全冒烟均通过；账号回归后的数据库备份为 `/srv/ai-kids-platform/production/backups/20260905T051738Z/platform.db`。
- [x] 修正运行手册中的生产备份命令，明确远程执行前必须设置 Node 24 PATH。

### 2026-09-05 教师成员名册页面修复发布

- [x] 页面回归发现教师进入“成员管理”时请求了无权限的机构全量名册，页面显示“无账号管理权限”。
- [x] 已修复机构端页面：机构管理员继续读取全量机构成员；教师改为读取 `org/users?role=STUDENT`，仅用于选择学生加入自己负责的班级；教师仍不显示创建账号、批量导入等管理区域。
- [x] 本地四端构建通过；`scripts/p4-o10-teacher-class-duty.mjs` **9/9** 通过。
- [x] 生产发布 commit `27e71a4b092fadeb9ae50af3b2aa24e657fdc3ce`，release `20260905T053145Z`；发布前备份 `/srv/ai-kids-platform/production/backups/20260905T053201Z/platform.db`。
- [x] 生产页面回归 **12/12**：机构管理员登录 / 成员管理 / 创建账号 / 批量导入 / 班级课堂，教师登录 / 学生名册 / 无账号创建与批量导入 / 班级课堂，学生工作台 / 无机构导航。
- [x] 发布后健康检查、四端公网入口 **4/4**、敏感路径安全冒烟 **14/14** 全部通过。

### 2026-09-05 页面级功能验收闭环

- [x] 机构管理员页面已完成账号生命周期验收：创建临时学生、批量导入（预览与整批提交）、重置密码、停用账号。
- [x] 教师页面已完成教学交付验收：创建临时班级、加入 `student-1`、配置已授权课时、开始课堂、结束课堂并归档。
- [x] 学生页面已完成创作交付验收：创建项目、打开魔法画布、保存画布版本、确认版权并提交作品。
- [x] 本轮页面级功能检查 **12/12 通过**；验收时间为 2026-09-05 13:49 CST。
- [x] 本轮创建的 5 个临时账号已软删除，临时班级已归档，没有遗留进行中的课堂；提交作品与项目保留为历史验收数据，未直接删除生产关联记录。
- [x] 该项完成后，线下机构交付前的账号、授课、学生创作与提交页面验收闭环已建立；剩余观察项仍为 P9-D02 24 小时观察，不提前宣称完成。
### 2026-09-05 P9-R02 Feature Flag 灰度能力开发（本地完成，待统一核验与发布）

- [x] 服务端新增 `feature_flags` 配置表，支持总开关、默认开关、稳定灰度比例、机构白名单、用户白名单；不存在业务默认放行，关闭状态可无发布回滚。
- [x] 新增 `GET /api/feature-flags`，由服务端按“用户白名单 → 机构白名单 → 默认开关 → 稳定灰度”统一计算当前登录用户结果，不向前端暴露配置细节。
- [x] 新增平台超管 API：`GET/POST/PATCH/DELETE /api/admin/feature-flags`，校验 key、比例和白名单规模；创建、修改、删除均写入操作审计。
- [x] 平台管理台新增“灰度开关”页面，可创建、编辑、删除和查看 Flag，支持机构 / 用户 ID 多行录入。
- [x] 本地验证：Node v24.19.0 语法检查通过；`node scripts/p9-r02-feature-flags.mjs` 隔离 SQLite 矩阵 3/3 通过；四端 `pnpm build` 通过；`git diff --check` 通过。
- [ ] 尚未发布生产；按用户要求跳过本轮机构交付核验，待功能批次结束后统一执行页面、API、权限和生产回归。
- [ ] 尚未将真实 AI、支付、公开作品等业务接入任何 Flag；本项只提供受控开关基础设施。

### 2026-09-05 P9-R03 平台管理员权限逐域收紧（本地完成，待统一核验与发布）

- [x] 定义八个业务域权限码：`ADMIN_ORGANIZATIONS`、`ADMIN_COURSES`、`ADMIN_WORKS`、`ADMIN_BILLING`、`ADMIN_CONTENT`、`ADMIN_ANALYTICS`、`ADMIN_FEATURE_FLAGS`、`ADMIN_AUDIT`。
- [x] 服务端在 `/api/admin/*` 入口统一按路径业务域授权；无权限统一返回 HTTP 403、错误码 `PERMISSION_DENIED`，不再仅凭 `SUPER_ADMIN` 角色放行；未知管理 API 默认落入 `ADMIN_CONTENT`。
- [x] 默认 `root` 保留完整权限；平台管理员编辑权限时保留最后一个有效平台管理员保护，禁止将唯一有效管理员清空全部业务域权限；禁止自己停用。
- [x] 平台管理台按权限码隐藏菜单；直接访问无权限路由显示拒绝提示；管理员编辑页只提供八个正式业务域权限码。
- [x] 本地验证：`node scripts/p9-r02-feature-flags.mjs` 通过；`node scripts/p9-r03-platform-permissions.mjs` 八域允许 / 拒绝、跨域、未知管理 API 默认域、root、自己停用、最后管理员保护和审计记录通过；四端 `pnpm build` 通过；`git diff --check` 通过。
- [ ] 尚未发布生产、修改生产数据库或执行统一机构交付核验；待后续功能批次完成后统一执行。

### 2026-09-05 官网首页 Inner Circle 视觉与交互改造（本地完成，待统一核验）

- [x] 官网首页改为单屏、无原生滚动、wheel / touch 手势驱动的 scrollProgress 0 → 3.5 交互；保留其他官网路由、预约演示、法律页面与统计同意能力。
- [x] 新增首页视频 scrub、鼠标视差、字符级标题退出、课程信息 pill tiles、第二屏上升面板、圆柱文字 drum、底部 marquee 与移动端全屏菜单。
- [x] 视觉适配 AI 魔法学院业务：使用品牌中文文案、AI 创作课堂、VibeCoding、阿飞与机构开课入口；采用洋红 / 酒红 / 白色体系，不引入紫色或靛蓝。
- [x] 首页视频依赖外部素材：两条 motionsites CDN 视频；素材加载失败时页面仍保留色块与文字层，不阻塞导航。
- [x] 首页 SEO title、description 与站点 theme-color 已同步到新首页定位；内部测试环境仍遵循 noindex。
- [x] 本地验证：Node v24.19.0 下四端 pnpm build 通过；首页构建产物生成成功。
- [ ] 尚未发布生产；尚未进行最终统一核验、真实设备矩阵或外部视频 CDN 稳定性核验。


## 11. P10｜文件 / 媒体安全上传（2026-09-05，开发中）

本阶段按用户确认开始：先完成安全上传功能开发，暂不修改生产业务数据，不进行本轮统一核验，不发布未完成变更。专项设计与接口细节见 `docs/p10/P10-file-media-secure-upload.md`。

- [x] 服务端 multipart 上传入口：平台 `POST /api/admin/file-assets/upload`、机构 `POST /api/org/file-assets/upload`。
- [x] 文件名、扩展名、MIME、文件签名、大小、路径隔离和 UUID 存储键校验。
- [x] WebP / OGG 签名识别；拒绝 NUL、控制字符、路径分隔符、盘符、双点和危险扩展名。
- [x] 上传拒绝、成功、下载行为审计；复用 `file_access_grants` 做访问控制。
- [x] `INTERNAL_PROXY` 授权下载改为 Web 根目录外受保护文件流，支持安全响应头和 Range。
- [x] 可配置 ClamAV 扫描；生产环境默认扫描器缺失 / 不可用时拒绝上传，并清理已写入文件。
- [ ] 上传频率、并发数和用户 / 机构容量配额。
- [ ] 平台 / 机构前端上传组件、进度与错误提示。
- [ ] 隔离数据库专项测试和最终统一核验。
### 2026-09-05 P4 教师点评闭环与 AI 异步队列恢复发布
...
- [x] 队列失败专项测试：`scripts/p4-o13-ai-generation-failure.mjs` 验证 provider 不可用时重试 2 次后 `FAILED`、不扣积分、只写 1 条失败用量记录、worker 释放。
...
- [x] 完成真实课堂任务闭环：在活动班级绑定已存在课时，学生开始任务、关联既有项目首次提交；教师驳回并写入反馈，学生重新提交；教师再次审核通过并评分；任务最后关闭。创建的验收任务均以验收后关闭结束，未修改成员、课程或账号。

### 2026-09-05 P1-P4 功能阶段验收与发布确认

- [x] P1 教务与项目基础链路、P2 画布与版本能力、P3 反馈 / 作品墙 / AI mock 基础闭环、P4 非画布真实业务闭环均已有代码、验收脚本和生产验证证据。
- [x] 本轮重新执行 P4 核心专项：P4-O09 **9/9**、P4-O10 **9/9**、P4-O11 **10/10**、P4-O12 队列恢复通过、P4-O13 失败重试通过、P4-O14 成员生命周期 **10/10**、P4-O15 取消清理通过；四端生产构建通过。
- [x] 生产已发布并运行当前 release `20260905T111322Z`（commit `eabb231`）；发布后 health、四端入口、安全冒烟、SQLite 完整性、备份与服务状态均通过。
- [x] P4 已按当前线下机构受控交付范围收口。P4-A07 / P4-O09 的真实 OSS、外部下载代理、阿飞微信通道、失败重试高可用和课节定时提醒等外部 / 扩展能力不冒充完成，分别保留至 P6 或后续需求；黑客松、公开内容治理、正式法律合规按用户决策暂缓。


### P6-A01 用户决策补充（2026-09-05）

- 用户确认允许学生创作内容外发；平台策略默认开启，平台超管可关闭，机构端只读。
...
## 2026-09-06 AI 渠道 401 修复与生产发布

- 根因：AI 渠道页面的“测试连接 / 读取模型”只把 `channelId` 和 Endpoint 发给后端，没有携带当前刚输入但尚未保存的渠道 Key；服务器因此使用旧 Key 或找不到对应渠道 Key。生产库当时也未保存用户页面中的“生视频”渠道，模态路由只有 `TEXT`。
- 修复：测试和读取模型请求临时优先使用当前表单 Key；保存策略时从普通 JSON 中剔除 `apiKey`，密钥只写入服务器受限加密文件。
- Commit：`6ac323e`；生产 release：`/srv/ai-kids-platform/production/releases/20260906T154630Z`。
