...
> 创建日期：2026-09-02  
> 当前阶段：**非画布真实业务闭环已完成本轮开发；当前执行“线上受控内部测试发布”。备案、正式公开合规后置；网站部署到 `iicili.cyou` 仅供团队内测，必须使用独立测试数据库、Basic Auth / 等效访问控制，并保留旧站与配置可回滚。P5-W08 仍为准备稿，真实 AI / 文件 / 支付 / 微信等外部服务继续等待业务方决策。**
...
### 1.1 当前唯一下一步（2026-09-03：用户授权改为线上受控内部测试发布）

> **执行口径：**用户已明确备案后续再做，并授权直接部署到线上网址 `https://iicili.cyou/` 供团队内部测试；这不是正式公开上线。发布必须先备份旧站、Nginx、systemd、配置和数据库文件，使用独立测试 SQLite（不得读取或写入真实线上业务库），启用 Basic Auth / 等效访问控制、noindex 和内测标识，再执行发布后的浏览器回归。当前本地 release 已准备完成，但真实服务器 SSH 入口从本执行环境不可达，因此暂不能声称已完成线上切换。

- [-] **线上受控内部测试发布与浏览器回归。**
  - 目标地址：`https://iicili.cyou/`；只供团队内部测试，不作为正式公开服务。
  - 发布前必须备份旧 `/opt/learning-platform/dist`、Nginx、systemd、环境配置及线上数据库文件；新服务只能使用独立测试 SQLite，不得迁移、覆盖或写入真实线上业务数据。
...
当前 P9 内测上线代码 / 文档基线已完成；用户已授权把它发布到 `iicili.cyou` 作为线上受控内部测试站，但本轮尚未完成真实服务器切换：`39.106.183.200:22` 从执行环境连接超时，且不得在没有备份与隔离测试库的情况下覆盖旧站。备案、正式公开服务和生产数据库迁移仍后续处理。
...
- [-] **P5-W08 协议 / 隐私 / 未成年人说明。**（代码闭环完成，正式生效待外部确认）
  - 优先级：P1
  - 完成记录（2026-09-03）：
  ...
  - 下一步：内部测试阶段继续使用明确标注的准备稿；正式公开上线前再确认主体、备案信息、正式生效日期和协议正文，替换准备稿并重新运行临时 SQLite 验收与四端构建。
...
- [-] **P5-W08 用户协议、隐私政策、儿童 / 未成年人说明**（代码闭环完成，正式生效待外部确认）
  ...
  - 阻塞 / 下一步：正式备案主体、正式生效日期和法务确认正文未提供；确认后替换准备稿并重新验收，不能以当前文案宣称已完成正式合规。
...
> 请先读取 `D:\学习平台\platform-v2\docs\AI少儿编程平台-完整上线执行总控.md` 与 `D:\学习平台\platform-v2\docs\AI少儿编程平台-P4-非画布真实业务缺口清单.md`，以“当前唯一下一步”和未勾选事项为准推进。当前优先级是 **P9-I01～P9-I06：受控内部测试上线**，备案、正式域名和公开生产合规后续再做。先核对代码、部署现状和文档是否一致；完成任何事项后，必须更新总控的勾选、完成记录、验证方式和变更日志。除非用户重新明确授权，禁止修改 `D:\学习平台\platform-v2\packages\canvas`，禁止触碰真实线上数据库，禁止伪造真实 AI、支付、文件、微信、短信、邮件或客户端能力。
