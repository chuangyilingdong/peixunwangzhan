...
- [ ] **当前唯一下一步：P4-O02 机构账号 / 成员完整管理。**
  - 优先级：P0
  - 选择原因：机构首页已能识别负责 / 授权班级，下一步应把教师授权、成员状态、调班和账号管理统一成可操作、可审计的闭环。
  - 目标：补齐批量导入预览与失败报告、调班 / 授权、禁用立即失效、重置密码和操作审计，并统一教师授权班级在班级、作品等教务接口中的读取与管理规则。
  - 验收重点：重复登录名 / 手机号策略明确；教师不能越权访问未授权班级；禁用账号会立即失效；批量导入失败可回滚；临时 SQLite 正反向 API、P3 回归、四端构建和总控更新通过。
...
- [x] **P4-00：完成“非画布页面壳层 → 真实业务能力”的接口/数据缺口清单，并按 P0 顺序开始补齐第一批真实页面。**
  ...
  - 完成记录（2026-09-02）：
    ...
    - [-] 剩余：P4-01、P4-02 第一批与 P4-03 两批已完成；黑客松 / 运营活动已按用户决策取消，当前按缺口清单推进 P4-O02 机构账号 / 成员完整管理；在线计费 / 作品治理及外部能力继续保留在对应后续批次。
