...
> 创建日期：2026-09-02  
> 当前阶段：**非画布真实业务闭环已完成本轮开发；当前优先进入“受控内部测试上线”阶段。备案、正式域名、品牌邮箱与公开服务合规不阻塞内部测试，但阻塞正式对外公开上线；P5-W08 仍为准备稿，真实 AI / 文件 / 支付 / 微信等外部服务继续等待业务方决策。**
...
### 1.1 当前唯一下一步（2026-09-03：内测代码基线已完成，执行真实隔离节点部署）

> **执行口径：**P9-I01～P9-I06 的代码、部署模板、运行手册和临时 SQLite 验收已完成；下一步是在新建、隔离的内测主机或内网节点按 `deploy/internal-test/RUNBOOK.md` 执行真实部署和浏览器回归。ICP / 公安备案、正式品牌域名、品牌邮箱和正式法务文本暂不作为内测阻塞项；但内测网址必须受控访问，且不能把准备稿、`local-mock` 或未接入的外部能力宣传为正式服务。

- [ ] **真实隔离内测节点部署与浏览器回归。**
  - 由具备服务器权限的人员在新建、隔离的内测主机 / 内网节点执行；不连接旧服务器，不修改 `iicili.cyou`，完成后仅称为“内部测试环境”。
  - 按 `deploy/internal-test/RUNBOOK.md` 完成 Nginx Basic Auth / VPN / IP 白名单、独立测试库、systemd、备份和五类角色浏览器回归；本项未完成前不扩大访问范围。
