...
- [-] **P4-03：完成通知与宣传物料第一批真实闭环，并登记外部能力边界。**
  - 优先级：P0
  - 完成记录（2026-09-02）：
    - [x] 新增 `notifications`、`notification_recipients`、`promo_materials`、`promo_material_assignments`、`promo_material_events` 表。
    - [x] 平台端 `/inbox`、`/materials` 接通真实 API；支持平台通知草稿、按机构/角色发布、撤回、置顶、跳转，以及宣传物料元数据创建、启停和机构授权。
    - [x] 机构端 `/inbox`、`/materials` 接通真实 API；支持平台公告/机构通知接收、单条/全部已读、可见物料查看、使用事件和服务端下载前置校验。
    - [x] 权限与隔离：平台通知写操作仅 `SUPER_ADMIN`；机构通知写操作仅 `ORG_ADMIN`；物料按机构可见范围过滤；未配置真实资源地址时返回 `MATERIAL_RESOURCE_NOT_CONFIGURED`，不伪造上传或下载。
    - [x] 验证：后端语法通过；19 项 P4-03 API 正反向场景通过；P3 API 回归 46 项通过；四端生产构建通过；`git diff --check` 通过。
    - [ ] 剩余：通知定时发布、投递失败重试、邮件/短信/微信通道、通知模板、真实文件上传/OSS/下载代理、物料统计详情、学生端通知中心、阿飞提醒。
  - 边界：本批不修改 `packages/canvas`，不伪造外部通知、文件存储或下载能力，不部署线上环境。
- [x] **P4-00：完成“非画布页面壳层 → 真实业务能力”的接口/数据缺口清单，并按 P0 顺序开始补齐第一批真实页面。**
  - 优先级：P0
  - 完成记录（2026-09-02）：
    - [x] 已产出并入库 `docs/AI少儿编程平台-P4-非画布真实业务缺口清单.md`，覆盖平台端 12 入口、机构端 14 入口、学生端 10 入口、官网 8 入口。
    ...
    - [-] 剩余：P4-01、P4-02 第一批已完成；继续按缺口清单补齐 P4-01 的在线计费 / 作品治理、P4-03 通知物料和 P4-04 活动与外部能力。
  - 目标：把目前明确标注“待接入”的平台端、机构端、学生端页面，逐页映射到数据表、后端接口、权限、交互、验收场景，防止只扩展菜单不形成业务闭环。
  - 依赖：本文件、`docs\AI魔法学院基准`、现有后端路由与数据库 schema。
  - 首批建议顺序：**平台用户 / 课程管理 → 机构账号与成员 → 课程中心与开通 → 积分套餐、充值、用量 → 通知与宣传物料 → 作品数据与黑客松**。
  - 验收：
...
- [x] 平台端已扩展以下非画布入口：平台概览、机构管理、平台用户、平台课程、课程广场、作品库、黑客松、计费与模型、素材与物料、站内信、平台管理员。
- [x] 机构端已扩展以下非画布入口：机构首页、站内信、班级管理、账号管理、课程中心、作品社区、作品数据中心、积分套餐、学员开通、积分充值、积分用量、宣传物料、黑客松、阿飞提醒。
...
- [ ] **P4-01 统一领域状态机、枚举与错误码**
  - 优先级：P0
  - 实现范围：统一机构、用户、班级、课堂、项目、作品、开通单、充值单、生成任务、通知、黑客松等状态及允许转换。
...
- [ ] **P4-A01 平台概览真实数据**
  ...
  - 验收：指标都有明确统计口径、时区、数据源和更新时间；无数据不显示伪造数字；数据权限仅限平台超管。
- [ ] **P4-A02 机构管理完整闭环**
  - 优先级：P0
  - 页面：机构管理。
...
- [ ] **P4-A09 黑客松 / 活动管理**
  - 优先级：P1
  - 页面：黑客松。
  - 实现范围：活动创建、报名条件、活动时间、赛题、投稿、评审、排名、公示、获奖通知。
  - 验收：报名、投稿截止时间由服务器判定；同一作品 / 用户重复提交规则明确；评审过程可追溯。
...
- [ ] **P4-O05 作品社区运营闭环**
  - 优先级：P1
  - 页面：作品社区。
  - 实现范围：提交审核、点评、发布 / 下架、精选、评论 / 点赞（若启用）、举报、作品版权 / 授权确认。
  - 验收：教师只处理负责班级作品；未审作品不公开；评论功能若上线必须有审核、屏蔽、举报与未成年人保护策略。
- [ ] **P4-O06 作品数据中心**
  - 优先级：P1
  - 页面：作品数据中心。
  - 实现范围：按班级 / 课程 / 学生统计完成数、提交率、发布率、反馈完成率、创作活跃、AI 用量和导出。
  - 验收：指标可下钻到原始记录；统计范围受权限限制；导出含必要脱敏和操作日志。
- [ ] **P4-O07 套餐、学员开通与席位管理**
  - 优先级：P0
  - 页面：积分套餐、学员开通。
  - 实现范围：套餐查看、机构购买 / 开通记录、学生席位分配、有效期、停用、续费、临近到期提醒、占用统计。
  - 验收：超出可用席位无法开通；停用或到期后的学生权限符合明确规则；所有变更留痕。
- [-] **P4-O08 积分充值、用量和对账**
  - 优先级：P0
  - 页面：积分充值、积分用量。
  ...
  - 完成记录（2026-09-02）：
    ...
    - [ ] 剩余：冻结金额、人工调整、退款 / 冲正、支付回调、导出对账、按课堂 / 学生 / 模型的完整筛选与流水复算验证。
- [-] **P4-O09 机构站内信、宣传物料与阿飞提醒**
  ...
  - 完成记录（2026-09-02，第一批）：
    - [x] 机构 `/inbox` 已接通平台/机构站内通知、按机构隔离、已读/全部已读；机构通知仅管理员可发送。
    - [x] 机构 `/materials` 已接通物料可见列表、使用事件和资源地址前置校验。
    - [ ] 剩余：到期/余额/课堂/审核自动提醒、忽略状态、定时/失败重试、阿飞微信通道、真实上传与下载代理。
- [ ] **P4-O10 机构黑客松参与与评审工作台**
  - 优先级：P1
  - 实现范围：活动报名、学生报名名单、作品投稿、教师初审、活动通知、排名查看。
  - 验收：机构只能管理自身参赛数据；活动规则 / 截止时间服务端校验；作品状态与活动状态一致。

### 4.3 学生端：非画布学习、个人与作品体验

- [-] **P4-S01 学习首页真实学习任务与进度**
  - 优先级：P0
  - 页面：学习首页、我的课程。
  ...
  - 完成记录（2026-09-02）：
    ...
    - [ ] 剩余：学习首页 `/dashboard` 的任务聚合、老师通知和继续创作入口待补。
- [ ] **P4-S02 我的项目管理（画布外层）**
  - 优先级：P0
  - 页面：我的项目。
...
- [-] **P4-S05 AI / 魔法石中心**
  ...
  - 完成记录（2026-09-02）：
    - [x] `/credits` 已实现套餐、周期额度、剩余额度、魔法石、课堂会话和用量明细筛选的真实页面。
    - [ ] 剩余：独立 AI 能力中心、任务重试 / 失败详情、素材使用记录和真实 AI 服务待补。
- [-] **P4-S06 个人账号与安全设置**
  - 优先级：P0
  - 页面：个人账号。
  ...
  - 完成记录（2026-09-02）：
    ...
    - [ ] 剩余：头像、监护人信息、隐私设置、账号注销和数据请求入口待补。
- [ ] **P4-S07 帮助、下载与反馈渠道**
  - 优先级：P1
  - 页面：帮助与下载。
...
- [-] **P4-C03 通知领域模型与投递中心**
  ...
  - 完成记录（2026-09-02，第一批）：
    - [x] 已建立通知主表与接收者投递表，支持草稿、立即发布、撤回、按机构/角色投递、置顶、跳转、已读和唯一投递约束。
    - [x] 平台/机构收件箱及权限隔离已通过 `19 pass / 0 fail` API 验收。
    - [ ] 剩余：定时发布、异步队列、失败重试与投递状态、模板及邮件/短信/微信渠道。
