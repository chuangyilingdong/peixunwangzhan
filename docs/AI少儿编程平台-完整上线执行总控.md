...
### 1.1 当前唯一下一步（2026-09-03：线上受控内部测试已发布，转入真实服务器 UAT）

> **执行口径：**用户已明确备案后续再做，并授权直接部署到线上网址 `https://iicili.cyou/` 供团队内部测试；这不是正式公开上线。已完成旧站保留、独立测试 SQLite、Basic Auth、noindex、内测标识和可回滚发布。备案、正式公开合规、真实 AI / 文件 / 支付 / 微信等外部服务仍不在本轮范围。

- [x] **线上受控内部测试发布与浏览器回归基线。** ✅ 2026-09-03
  - 目标地址：`https://iicili.cyou/`；当前仅供团队内部测试，不作为正式公开服务。
  - 真实服务器已完成旧站备份与隔离发布：备份目录 `/srv/ai-kids-platform/backups/pre-internal-test-20260903T165955Z`；新 release `/srv/ai-kids-platform/internal-test/releases/20260903T172458Z`；当前软链接 `/srv/ai-kids-platform/internal-test/current`。
  - 新服务 `learning-platform-internal-test` 已 active，API 仅监听 `127.0.0.1:8788`，使用 `/srv/ai-kids-platform/internal-test/data/platform.db` 独立测试 SQLite；旧服务 `learning-platform` 保留，可回滚。
  - Nginx 已启用 HTTPS、Basic Auth、`X-Robots-Tag: noindex, nofollow, noarchive`、`X-Internal-Test: true`、robots 禁止索引和 SPA fallback；`nginx -t` 成功并已 reload。
  - 线上验收已通过：`/`、`/admin/`、`/org/`、`/student/` 均 HTTP 200；`/api/health` 返回 `status=ok`；未认证访问返回 HTTP 401；API 8788 仅回环监听；页面含“内部测试环境 · 不代表正式服务”。
  - 安全边界：未读取、复制、迁移、备份或写入旧站真实数据库 `/opt/learning-platform/server/data/learning-platform.sqlite`；未扩大为公开服务。
...
> 请先读取 `D:\学习平台\platform-v2\docs\AI少儿编程平台-完整上线执行总控.md` 与 `D:\学习平台\platform-v2\docs\AI少儿编程平台-P4-非画布真实业务缺口清单.md`，以“当前唯一下一步”和未勾选事项为准推进。当前优先级是 **线上受控内部测试 UAT 与缺陷闭环**；备案、正式域名和公开生产合规后续再做。先核对代码、部署现状和文档是否一致；完成任何事项后，必须更新总控的勾选、完成记录、验证方式和变更日志。除非用户重新明确授权，禁止修改 `D:\学习平台\platform-v2\packages\canvas`，禁止触碰真实线上数据库，禁止伪造真实 AI、支付、文件、微信、短信、邮件或客户端能力。
