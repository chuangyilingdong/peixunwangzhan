...
- [-] **线上受控内部测试发布与浏览器回归。**
  ...
  - 当前阻塞：本执行环境对 `39.106.183.200:22` 仍连接超时，尚无可用服务器控制通道；`80/TCP`、`443/TCP` 可达。通过浏览器只读检查 `https://iicili.cyou/` 已确认仍是旧站（标题“创易灵动学习画布”），未出现“内部测试环境 · 不代表正式服务”、Basic Auth 或新四端路径，因此不得视为已部署。已完成本地构建和 24/24、11/11、30/30 临时 SQLite 验收。拿到可用 SSH / 云控制台通道后，按 runbook 执行备份、上传、切换、健康检查和浏览器回归。
...
| 2026-09-03 | 线上内测发布再次预检：域名 DNS 仍指向 `39.106.183.200`；80/443 TCP 可达，22/TCP 从本执行环境超时；浏览器只读打开 HTTPS 后确认线上仍为旧站“创易灵动学习画布”，未切换到本平台内测版本。 | 未修改线上文件、未重启线上服务、未触碰真实线上数据库；发布继续阻塞于服务器控制通道，待 SSH 或 ECS 云服务器终端可用后执行备份、隔离库初始化、发布和浏览器 UAT。 |
