...
- [ ] **当前唯一执行队列：内测产品与技术收口。**
  ...
  - P1：P8-Q01、P8-Q02 已通过；P4-C01 已于 2026-09-04 完成；当前继续推进：① P8-Q05 可用浏览器 / 视口回归边界记录；② P4-01 状态机 / 枚举 / 错误码统一；③ P4-03 筛选 / 分页 / 排序 / 空态 / 导出规范；④ 平台 / 机构管理员 / 教师 / 学生角色 UAT 与缺陷收口。不得虚构独立 Chrome / Edge / Safari 或真机结果。举报、申诉、违规 / 内容审核、监护人和正式法律 / 合规按用户决策 `[~]` 暂缓。
...
- [x] **P4-C01 后端 RBAC / 资源归属校验全覆盖** ✅ 2026-09-04
  ...
  - 完成记录（2026-09-04）：
    ...
    - 遗留风险或下一步：P4-C01 代表当前服务端关键权限边界已覆盖并有专项回归；继续执行 P4-01 状态机 / 枚举 / 错误码统一，随后执行 P4-03 列表规范和角色 UAT。正式公开前仍需恢复访问控制并重新评估暂缓的举报、申诉、违规 / 内容审核、监护人和正式法律 / 合规范围。
- [x] **P4-C02 审计日志可查询**
...
- [ ] 下一项：P4-01 状态机、枚举、错误码与异常提示统一；P8-Q05 继续保留当前环境无法完成独立浏览器 / 真机矩阵的边界说明。
