...
- [-] **P4-04：黑客松 / 运营活动第一批真实闭环。**
  - 优先级：P0
  - 下一批目标：优先实现不依赖外部平台的赛季、参与范围、作品投稿与审核状态机，并接通平台 / 机构端真实列表和操作页面。
  - 验收重点：活动与投稿截止时间由服务端判定；机构数据隔离；状态迁移、审核人与审计日志可追溯；临时 SQLite 正反向 API、P3 回归和四端构建通过。
  - 外部边界：微信提醒、在线支付、真实上传 / OSS、真实 AI 继续作为外部决策或 P6 基础设施事项，不以占位按钮伪装完成。
...
- [x] **P4-00：完成“非画布页面壳层 → 真实业务能力”的接口/数据缺口清单，并按 P0 顺序开始补齐第一批真实页面。**
  ...
  - 完成记录（2026-09-02）：
    ...
    - [-] 剩余：P4-01、P4-02 第一批与 P4-03 两批已完成；当前按缺口清单推进 P4-04 活动状态机，在线计费 / 作品治理及外部能力继续保留在对应后续批次。
