...
### 1.1 当前唯一下一步（2026-09-03 新决策：先内部测试上线，备案后续再做）

> **执行口径：**本轮目标是先部署一个不对外公开的内部测试环境，而不是正式公开上线。ICP / 公安备案、正式品牌域名、品牌邮箱和正式法务文本暂不作为内测阻塞项；但内测网址必须受控访问，且不能把准备稿、`local-mock` 或未接入的外部能力宣传为正式服务。
