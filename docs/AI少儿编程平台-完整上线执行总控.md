...
- [-] **当前唯一下一步：P4-O01 机构首页真实经营看板第一批闭环。**
  - 优先级：P0
  - 选择原因：现有 `GET /api/org/overview` 和机构首页已经读取真实数据，但机构管理员与教师仍共用机构全量统计，尚未满足教师仅查看授权班级的范围要求；该项可复用现有机构、班级、课堂、作品、积分和通知数据，不依赖支付、OSS、真实 AI 或其他外部平台。
  - 下一批目标：区分机构管理员经营视图与教师教学视图；补齐近期课堂、待点评作品、未读消息、合同 / 席位 / 余额预警和可追溯统计口径；教师统计严格限制到本人负责或授权班级。
  - 验收重点：跨机构与跨教师范围 API 均被拒绝或不可见；各统计值可由明细复算；空数据不伪造；临时 SQLite 正反向 API、P3 回归和四端构建通过。
  - 画布边界：继续冻结 `packages/canvas`。
- [~] **P4-04：黑客松 / 运营活动不再实施。**
...
- [x] **P4-00：完成“非画布页面壳层 → 真实业务能力”的接口/数据缺口清单，并按 P0 顺序开始补齐第一批真实页面。**
  ...
  - 完成记录（2026-09-02）：
    ...
    - [-] 剩余：P4-01、P4-02 第一批与 P4-03 两批已完成；黑客松 / 运营活动已按用户决策取消，当前按缺口清单推进 P4-O01 机构首页经营看板与教师范围隔离；在线计费 / 作品治理及外部能力继续保留在对应后续批次。
...
- [x] 平台端已扩展以下非画布入口：平台概览、机构管理、平台用户、平台课程、课程广场、作品库、黑客松历史壳层、计费与模型、素材与物料、站内信、平台管理员；其中黑客松已于 2026-09-02 决定不做，不再属于产品功能范围。
- [x] 机构端已扩展以下非画布入口：机构首页、站内信、班级管理、账号管理、课程中心、作品社区、作品数据中心、积分套餐、学员开通、积分充值、积分用量、宣传物料、黑客松历史壳层、阿飞提醒；其中黑客松已于 2026-09-02 决定不做，不再属于产品功能范围。
