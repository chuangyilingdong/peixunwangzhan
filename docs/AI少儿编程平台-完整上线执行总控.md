...
- [ ] **P9-I01 内部测试环境部署基线。**
  - 优先级：P0（当前唯一下一步）
  - 明确服务器 / 进程 / 静态产物 / API / SQLite 测试库的目录与启动停止命令；固化 Node 20+ / pnpm 11 工具链和环境变量清单。
  - 只允许从已推送 commit 构建；不得直接覆盖旧线上 `dist`，不得使用 `packages\data\platform.db` 作为测试库。
- [ ] **P9-I02 内部访问控制与不可索引。**
  - 优先级：P0
  - 在反向代理或 VPN / IP 白名单 / Basic Auth 中落实至少一种强制访问控制；未授权请求不得进入官网、三端或 API。
  - 测试环境默认 `noindex, nofollow, noarchive`，`robots.txt` 不允许抓取；页面显示“内部测试环境，不代表正式服务”。
- [ ] **P9-I03 测试数据、环境隔离与初始化。**
  - 优先级：P0
  - 测试数据库独立初始化、迁移、seed 和清理；测试账号按平台管理员 / 机构管理员 / 教师 / 学生 / 官网访客准备；严禁真实用户数据进入测试环境。
- [ ] **P9-I04 备份、恢复与回滚演练。**
  - 优先级：P0
  - 对测试数据库、配置、静态制品和日志做可读取备份；演练应用回滚与数据库恢复，记录 RPO / RTO、版本、执行人和结果。
- [ ] **P9-I05 内部 UAT 与缺陷闸门。**
  - 优先级：P0
  - 按平台超管、机构管理员、教师、学生、官网访客五类角色执行核心流程；P0 权限、租户隔离、课程、课堂、作品、预约、通知、积分和账号安全缺陷必须清零或有明确豁免。
- [ ] **P9-I06 内测运行手册与日志。**
  - 优先级：P1
  - 明确健康检查、日志查看、错误上报、发布窗口、停止服务、回滚和联系人；内测阶段不承诺公开 SLA，不接收外部真实业务。
...
| 2026-09-03 | 根据用户新决策重排上线优先级：备案暂缓不阻塞受控内部测试；新增 P9-I01～P9-I06 内测部署、访问控制、数据隔离、备份回滚、UAT 与运行手册闸门，并将正式域名 / HTTPS / 备案 / 公开服务移至正式公开上线阶段。 | 新增《AI少儿编程平台-内部测试上线执行清单.md》；尚未执行服务器部署，当前只完成口径与执行顺序梳理。 |
