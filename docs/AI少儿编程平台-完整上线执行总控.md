...
- [ ] **当前唯一下一步：P4-O04 课堂内 AI 能力控制与使用审计。**
  - 优先级：P0
  - 选择原因：班级、课程计划和课堂状态链路已具备真实数据基础，下一步补齐课堂内 AI 能力的服务端开关、额度限制、教师即时暂停和可追溯审计。
  - 目标与验收：文本 / 图像 / 音频 / 视频能力开关在服务端强制执行；关闭或超额时请求被拒绝；每次消耗可追溯到机构、课堂、用户、项目和任务，并继续使用临时 SQLite、保持画布冻结。
...
- [x] **P4-00：完成“非画布页面壳层 → 真实业务能力”的接口/数据缺口清单，并按 P0 顺序开始补齐第一批真实页面。**
  ...
  - 完成记录（2026-09-02）：
    ...
    - [-] 剩余：P4-01、P4-02 第一批与 P4-03 两批、P4-O01、P4-O02、P4-O03 已完成；黑客松 / 运营活动已按用户决策取消，当前按缺口清单推进 P4-O04 课堂内 AI 能力控制与使用审计；在线计费 / 作品治理及外部能力继续保留在对应后续批次。
...
- 对话交接确认（2026-09-02）：黑客松 / 运营活动不做，已记录为产品取消；本轮 P4-O02 代码与文档变更已提交并推送。每次后续完成事项必须同步更新本文件勾选、完成记录与变更日志。
