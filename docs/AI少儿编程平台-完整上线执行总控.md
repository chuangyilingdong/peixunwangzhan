...
- [ ] **当前唯一执行队列：内测产品与技术收口。**
  ...
  - P1：P8-Q01、P8-Q02 已通过；按以下顺序推进：① P8-Q05 可用浏览器 / 视口回归；② P4-C01 服务端 RBAC、资源归属与跨机构 / 跨学生越权覆盖；③ P4-01 状态机 / 枚举 / 错误码统一；④ P4-03 筛选 / 分页 / 排序 / 空态 / 导出规范；⑤ 平台 / 机构管理员 / 教师 / 学生角色 UAT 与缺陷收口。不得虚构独立 Chrome / Edge / Safari 或真机结果。举报、申诉、违规 / 内容审核、监护人和正式法律 / 合规按用户决策 `[~]` 暂缓。
