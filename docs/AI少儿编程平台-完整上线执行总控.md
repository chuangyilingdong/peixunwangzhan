...
> 创建日期：2026-09-02  
> 当前阶段：**非画布真实业务闭环已完成本轮开发；线上受控内部测试已发布，现按正式上线节奏推进质量、安全、隐私、运营和发布门槛收敛。用户已确认备案完成，备案后续材料与具体办理由用户自行处理；在正式门槛全部通过前，线上继续保持 Basic Auth、noindex、独立测试数据库和“内部测试环境”标识。真实 AI / 文件 / 支付 / 微信等外部服务仍必须有真实接入与验收，禁止用占位能力冒充正式服务。**
...
### 1.1 当前唯一下一步（2026-09-04：转入正式上线门槛收敛）
...
- [ ] **正式上线门槛收敛：质量、接口、安全、隐私与运营保障。**
  - P0：P8-Q03 API 集成测试扩展、P8-Q04 E2E、P8-Q06 性能与容量、P8-S01 安全基线、P8-S02 会话安全、P8-S03 多租户隔离、P8-S04 备份恢复、P8-S05 监控告警、P8-S06 发布回滚。
...
- [!] **外部 / 用户侧事项。**
  ...
  - P5-W08 正式协议正文、主体、联系人和生效信息仍需用户 / 法务最终确认后替换准备稿。
...
> 请先读取 `D:\学习平台\platform-v2\docs\AI少儿编程平台-完整上线执行总控.md` 与 `D:\学习平台\platform-v2\docs\AI少儿编程平台-P4-非画布真实业务缺口清单.md`，以“当前唯一下一步”和未勾选事项为准推进。当前优先级是 **正式上线门槛收敛：P8 质量、安全、隐私、运营与发布回滚**；备案已由用户确认完成但后续由用户自行处理。线上继续保持受控内测，未通过正式门槛前不得解除 Basic Auth / noindex 或宣传未接入能力。每次完成事项必须更新总控勾选、完成记录、验证方式和变更日志；继续禁止修改 `D:\学习平台\platform-v2\packages\canvas`、触碰真实线上数据库或伪造外部服务能力。
