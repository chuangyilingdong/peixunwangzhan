...
| P1 | P8-Q05 | **进行中（2026-09-04）**：真实 Chrome 152 / Edge 152 官网 12 路由 × 4 视口 **96/96** 通过；工作台 13 路由 × 桌面 / 390 视口修复共享布局横向溢出后 **52/52** 通过，四项回归与四端构建通过；Safari、真机和线上站点复验仍未覆盖，保持 `[-]` |
...
当前唯一执行顺序：
1. **P8-Q05**：本机 Chrome 152 / Edge 152 已完成官网与三端关键路由回归并修复移动横向溢出；剩余 Safari、真实移动 / 平板设备和线上生产站点复验，不得虚构未覆盖结果。
...
### 8.17 P4-O10 教师班级教务职责收敛（2026-09-05）

- [x] 明确线下机构职责边界：机构管理员管理机构账号与机构级事项；教师负责创建班级、将本机构学生加入 / 移出本人负责或获授权班级、配置已授权课程 / 课时并开展课堂。
- [x] 教师履行上述三项日常教务职责不再依赖 `MANAGE_CLASSES` / `MANAGE_MEMBERS` 额外权限；仍通过机构范围、班级负责人 / 教师授权关系和课程授权校验，禁止跨机构、跨教师班级和未授权课时操作。
- [x] 教师可读取本机构学生名册（仅 `role=STUDENT` 查询），机构成员全局创建、编辑、启停、密码和班级批量调整仍仅机构管理员可执行。
- [x] 机构端 `/classes` 已将加入 / 移出学生入口开放给教师，并明确教师是班级创建、学生编班和课程配置的责任角色；机构管理员保留机构级班级授权和必要的经营视图。
- [x] 临时 SQLite 验收脚本 `scripts/p4-o10-teacher-class-duty.mjs` **9 pass / 0 fail**：覆盖无额外教师权限、教师创建班级、读取学生、加学生、配置课时、非法教师、跨教师班级和学生越权；`node --check`、机构端生产构建、`git diff --check` 通过。
- [x] 本批未修改 `D:\学习平台\platform-v2\packages\canvas`，未触碰生产 SQLite，不接入外部通知通道，不把法律页 / 值班电话列为当前阻塞项。

## 8. 本轮总验收

（P4-00 / P4-01 / P4-02 / P4-03 / P4-O01 / P4-O02 / P4-O03 / P4-O04 / P4-O05 / P4-O06 / P4-O07 / P4-O08 / P4-S01 / P4-S02 / P4-S03 / P4-S04 / P4-S05 / P4-S06 / P4-S07）
...
### 2026-09-04 P8-Q07 角色 UAT：官网访客阶段

- [x] 官网访客 UAT 完成：12 个官网路由、未知路径回落、noindex 与内测横幅、公开法律/下载/作品/课程广场/课程详情、预约演示线索、6 类受保护 API 未登录拒绝全部通过；专项 18/18，无新增 P0/P1。
- [x] 证据：evidence/p8-q07/visitor-uat.txt、visitor-*.jpg；全程隔离临时 SQLite，未修改 packages/canvas，未触碰真实线上数据库。
- [ ] 下一步：P8-Q07 四角色 UAT 已完成，转入 P8-Q05 浏览器回归边界记录与内测缺陷收口；独立浏览器矩阵、真机与正式公开上线继续不宣称完成。
### 2026-09-04 P8-Q07 角色 UAT：学生阶段

- [x] 学生 UAT 完成：异常登录矩阵、跟随课堂无课堂拦截、自主练习 9 页巡检、跨学生项目隔离、机构 / 平台 API 与跨端 UI 拒绝、登出旧 Cookie 失效全部通过；专项 26/26，无新增 P0/P1。
- [x] 回归：RBAC 27/27、列表 50/50、P3 集成 52/52、E2E 54/54、四端生产构建通过；未修改 packages/canvas，未触碰真实线上数据库。
- [ ] 下一步：官网访客 UAT；举报、申诉、违规 / 内容审核、监护人、正式法律 / 合规继续 [~] 暂缓。
### 2026-09-04 P8-Q07 角色 UAT：教师阶段

- [x] 教师 UAT 完成：异常登录矩阵、7 个可见页面、5 个隐藏管理页边界、跨平台 / 学生端拒绝、7 类 API 越权和登出会话失效全部通过；真实浏览器页面错误 0。
- [x] 修复 P1 权限缺口：教师原可读取 `GET /api/org/billing/packages` 的套餐价格、积分、席位与能力配置；现仅 `ORG_ADMIN` 可读，教师返回 403 `ORG_ADMIN_REQUIRED`。保留教师读取积分用量汇总 / 明细的既有课堂扣积分链路，不扩大修复范围。
- [x] 回归：教师专项 19/19、RBAC 27/27、列表 50/50、P3 集成 52/52、E2E 54/54、四端构建与 `git diff --check` 通过；机构管理员套餐 / 用量接口复验 200。
- [x] 下一步已进入学生 UAT（见上节）；官网访客仍待执行；举报、申诉、违规 / 内容审核、监护人、正式法律 / 合规继续 `[~]` 暂缓。
