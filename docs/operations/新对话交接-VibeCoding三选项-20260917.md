...
> ⚠️ **这份已被第十一轮取代**（`docs/operations/新对话交接-VibeCoding功能按钮-20260917.md`）：
> 学生干活的地方**就是创作环境（dsh）那一个页面**，三个功能做成 dsh 输入框那一排按钮；
> 本文里「进去默认走平台自己的链路」「工作台里的三张选项卡片 + 让 AI 真的做出来」那套已经不适用
> （平台那套老工作台已删）。**后端链路与第四节、第六节的坑仍然有效。**
>
