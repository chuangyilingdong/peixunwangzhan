...
- 当前性质：生产已切换；本交接包服务线下机构交付。24 小时观察仍待截止复核；外部通知渠道、值班电话和外部升级联系人按范围决策不纳入当前建设。

> **边界声明**：生产 `AI_PROVIDER=local-mock`，仅为本地演示占位，不是真实 AI 服务；法律页不作为当前线下机构交付前置项；真实支付、OSS、短信、邮件、微信通道均未接入。本文不包含任何密码、密钥、Token 或 `.env` 内容。
...
- 机构管理员：创建 / 维护机构教师与学生账号，处理账号状态、班级授权和机构级经营事项；查看站内信、作品审核、物料和余额状态。
- 教师：创建班级，将本机构学生加入 / 移出本人负责或获授权班级，为班级配置已授权课程 / 课时，开展课堂并审阅学生作品；反馈只通过已实现的站内闭环。
...
当前平台不建设外部通知渠道、值班表、联系人和升级电话；以下仅保留内部异常分级与处置模板，由平台负责人 / 授权运维人员执行：
...
### 8.2 2026-09-05 线下机构交付闭环收口

- 教师职责已固化：创建班级、加入 / 移出本机构学生、配置已授权课程 / 课时、开课、课堂控制和结束课堂均按本人负责或获授权班级校验，不再要求 `MANAGE_CLASSES` / `MANAGE_MEMBERS`。
- seed 教师账号 `teacher-1` / `teacher-2` 均为空权限，避免演示账号掩盖真实职责边界。
- 隔离验收：教师职责 9/9、P4-C01 27/27、P8-Q03 52/52、P8-Q04 54/54；四端构建通过。
- 当前仍未完成：GitHub Actions 实际运行留证和最新 commit 的受控 production release 切换；不包含外部告警、值班电话、正式法律页。

## 9. 未完成清单

- [ ] GitHub Actions 至少一次实际运行留证；
- [ ] P9-D04 密钥轮换、Secret 台账、变更审计和通知渠道；
- [~] 24 小时观察与 7 / 30 天复盘：按用户决定不等待 24 小时窗口；保留已有健康、备份、恢复演练记录，后续按实际机构运行再复盘；
- [x] 外部通知渠道、值班人员和升级联系人：按线下机构平台范围决策，不纳入当前建设；
- [ ] 每月恢复演练与异机 / 对象存储副本。
