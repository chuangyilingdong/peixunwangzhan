...
代码提交：799783f（本地 HEAD = origin，已推送；**生产版就是它**）
生产：release 20260918T145658Z / commit 799783f（服务 learning-platform-production @127.0.0.1:8789）
      本版改动（799783f，用户口径：**常见问题页删三处文案 + 改按端三档**）：
      ...
      **静默退回写死的兜底**（页面看着正常、后台改了不再生效，p70/p115 都拦不住）。改成读 student 档，
      ...
      ①**`/faq` 换成黑底卡片式手风琴**：原来的浅底两栏 + `<details>` → 黑底居中页头 + 圆角 16px 卡片列表
