...
> 版本：2026-09-17（第八轮 · **已上线**）　｜　**生产 release `20260917T025334Z` / commit `87fd3ed`**
> 第八轮（本轮）集中攻 **VibeCoding 课堂的 AI 链路**（新会话**先读**
> `docs/operations/新对话交接-VibeCoding能力-20260917.md`）：
> **打通了网关的 `tools` / `tool_calls`** —— 这是本轮最大的一件事。之前我们的 OpenAI 兼容网关
...
> ⚠️ 两处**用户已表态、下一轮要先问清**：① 「秒进」我的方案（预热 + 关闲置回收）被用户否掉，
> 要重做；② dsh 的**网页搜索**要接到我们的网关（做法已查清：是 Anthropic 协议透传，不是搜索端点）。
> 全量守卫 **114/114**（新增 p103 DSML 过滤、p104 入口按课时类型放行、p105 收尾回收与预热接线）。
