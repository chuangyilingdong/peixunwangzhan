...
> 版本：2026-09-17（第九轮 · **已上线**）　｜　**生产 release `20260917T032154Z` / commit `a142ac0`**
> 第九轮（本轮）做的是上一轮交接点名的第一件事：**把 dsh 的网页搜索接到我们网关上**。
> 查清后确认原设想是错的 —— 那个插件调的**不是搜索接口**，而是 **Anthropic 协议的 `/messages`**
> （搜索是模型一跳里的服务端工具 `web_search_20250305`），所以加的是一条 **Anthropic 透传**
> （`/api/gateway/v1/search/messages`）：学生环境拿运行时密钥打进来，网关验签 → 过门禁 → 换成
> 渠道真密钥转发 → 记账。**渠道真密钥绝不进学生环境**（学生读得到自己的进程环境）。
> 身份/门禁/记账与聊天那条路共用同一套实现。已端到端实测：真密钥 + 照插件源码发的请求 →
> 200、10 条真实搜索结果、账本记下 2782/459 tokens 与折算成本 0.9236 分。
...
> 详见 `docs/operations/新对话交接-网页搜索与秒进-20260917.md`（**新会话先读这份**）第三节。
> 全量守卫 **115/115**（新增 p106 搜索透传）。
