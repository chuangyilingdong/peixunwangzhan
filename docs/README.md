...
> 版本：2026-09-16（第四轮 · **已上线**）　｜　**生产已部署**（release `20260916T130351Z` / commit `54ac4bd7`；
> 之后宿主侧的代理修复不涉及平台代码）。学生端 VibeCoding 迁 dsh（**学生看到的品牌是「灵动ai」**）：
> **创作环境 + 产物回传整条链已在真机上验完** —— 学生进环境、列产物、提交（网页/**PPT 等真二进制**都行）、
> 落进现有作品链路、机构端能看（**服务端把 Office 转成 PDF 当预览**）与下载原始文件；
> 学生端入口也已接线（**运行时可用时**课程中心才显示「进入创作环境」/「提交作品」，不可用则保持原样）。
> 仍然待办：① **文件扫描在这台机器上会拖垮上传**（内存不够，需按交接第四节定口径：
> 建议把扫描从「提交」挪到「发布」）② **前端还没在真浏览器里点过**（本机出口封 HTTPS）
> ③ 工作区预选 ④ 机器扩容。
> 见**最新交接** `docs/operations/新对话交接-dsh迁移-20260916-第四轮.md`（含只有真部署才暴露的两个 bug、
> 扫描内存问题、生产里留的测试数据、复核命令）。
> 整体 MVP / 1–7 项仍不能标为完成。**
