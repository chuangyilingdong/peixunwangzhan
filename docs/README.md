...
> 版本：2026-09-17（第十二轮 · 课堂按线框图改造 + 教学素材预览）
> **⚠️ 新会话先读** `docs/operations/新对话交接-课堂按线框图改造-20260917.md` ——
> 那一份是**最新**的：教师端「我的课堂」按 8 张线框图改成四个独立路由（列表 / 创建 / 详情 / 添加学生）、
> 后端只补了「列表分页 + 开始·解散预检」两处、后台字体统一到画布那套 `--cv-font`，
> 以及**浏览器实点抓出来的四个问题**（都在那一节）。
>
...
代码提交：f0c7824（本地 HEAD = origin，已推送）
生产：release 20260917T102638Z / commit f0c7824（服务 learning-platform-production @127.0.0.1:8789）
      生效内容：课堂四页按线框图改造 + 后端补分页与预检 + 后台字体统一 +
      教学素材查看器（pdf.js 画 canvas：无下载/打印/右键入口，快捷键拦死，可全屏讲课）
      部署后核验：BUILD-METADATA commit = f0c7824；active、NRestarts=0；
      ...
      上一版（可回滚）：release 20260917T102202Z / commit 3865a14
      整库备份：production/backups/20260917T102638Z/platform.db
