...
> 版本：2026-09-16（第五轮 · **已上线**）　｜　**生产 release `20260916T140008Z` / commit `bb425bd`**
> 本轮是**用户看界面提的 8 条产品口径调整**，全部已实现并部署（逐条见
> `docs/operations/产品口径调整-20260916.md`）：课包可见范围改「公开 / 私有」两值（含老库重建表迁移）、
> 平台预算能看出**超了多少**且工作台有明确提示、素材编辑器重排（生成框体不再要求上传素材）、
> 删掉「文字说明」素材类型、平台→机构授权**跟随合同日期**（不再填有效期）、
> 账号**唯一性校验 + 登录名只允许英文数字**、修复**素材在线预览全部失效**
> （根因是 X-Frame-Options: DENY 挡住了同源 iframe，应用与 nginx 两处都要放）、
> 机构端学员名单改为**筛选 + 翻页**（最新在前）。全量守卫 110/110 通过（含新增 p101）。
> 另外订正了 4 处**早就漂移**的测试断言（p6-learn-portal ×2、p81 ×2，不是本轮引入）。
> **下一轮专门修 bug 请看** `docs/operations/新对话交接-继续修bug-20260916.md` ——
> 里面写了现状与基线、怎么复现、**哪些地方最可能出问题（含这一轮所有界面改动都没在浏览器里看过）**、
> 以及几条「看着像 bug 其实是有意为之」的行为对照表。
> 上一轮（dsh 迁移）的交接见 `docs/operations/新对话交接-dsh迁移-20260916-第四轮.md`：
> 学生创作环境 + 产物回传已上线并真机验完，仍欠 ① 前端在真浏览器里点一遍 ② 文件扫描的口径
> （这台机器内存不够，扫描会把上传拖垮）③ 机器扩容。
> 整体 MVP / 1–7 项仍不能标为完成。**
