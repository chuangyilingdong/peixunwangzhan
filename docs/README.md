...
> 版本：2026-09-19（第十九轮 · 网站去掉 dsh + 客户端登录页按参考图重做 + 品牌落地）
...
> 客户端那个密钥卡点的修法与机理修正）**、**不许破的口径**（第十五～十八轮 25 条 ＋ 本轮新增 7 条）、
> **结构与关键文件**、**下一步**，以及**这一轮踩过的 11 个坑**。
> 其中三条最值钱：⭐ **网站侧不许再出现 `student/runtime/launch|deliverables|submit`**（
> 「网站上的 dsh 就不要了，以后 vibecoding 就是在客户端进行」，新守卫 **p117** 就是钉"一次 runtime 请求都不许打"）；
> ⭐ **上一轮"宿主起在登录门之前"的结论被修正** —— `backend.start()` 其实就在门后面，
> 而"写凭据文件也没用"那条实验**没有读方**（补丁层读的是 `apiKeyEnv`），所以修法落成
> **过门后 `resetHost()` 停一次宿主**（`runLingdongGate` 第三个参数）；⭐ **设计稿里烘着文案** ——
> 参考图直接裁来当素材会把图上文字一起带进去，而**所有断言都是绿的**（截图才看出来）。
>
> 紧挨着的上一层（**客户端的来龙去脉读它**）：
