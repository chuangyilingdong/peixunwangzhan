...
生产：release 20260913T115351Z / commit a515ff4（服务 learning-platform-production @127.0.0.1:8789）
...
8. **成本必须区分来源**：ESTIMATED、REPORTED、UNKNOWN、MOCK 各自保留语义；供应商账单匹配后的金额才是实际结算成本。机构实收、许可确认收入、结算成本和真实毛利分账核算；旧数据没有证据时保持 UNKNOWN，不能猜成本。操作见 `docs/operations/真实三账对照-20260914.md`。
