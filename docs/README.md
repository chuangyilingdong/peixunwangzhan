...
> 版本：2026-09-18（第十五轮 · 官网改版：品牌统一「灵动ai学院」/ 导航七项 / 三个 CMS 页面 / 首页重做）
> **⚠️ 新会话先读** `docs/operations/新对话交接-第十五轮-官网改版-20260918.md` ——
> 那一份写清了**这一轮改了什么（对应你给的 6 条）**、**我替你拍的三个板**（生产 CSP 挡住了样式提示词里
> 的外链字体、背景视频换成站内资产、没编造政策文件与课程名）、**上线顺序（官网内容迁移脚本必须排在
> 发布之前）**，以及还欠什么。
> ⚠️ 这一轮的改动**还在工作区、没有发布**（也还没提交），所以下面「二、现在的状态（生产）」写的仍是
> 上一版真实状态 —— 别把它和本轮的工作区改动混在一起看。
> 上一轮那一份（`docs/operations/新对话交接-第十四轮-官网改动前-20260918.md`）把**上一版生产状态**、
> **不许破的五条口径**（尤其：平台侧只有"授权次数"没有"算力额度"、额度只观测不拦人、内部成本口径
> 不上公开面）与**动官网的三个坑**串在一起，**仍然有效**。
> 再上一轮（`docs/operations/新对话交接-第十三轮梳理-20260917.md`）把**第十三轮的五条线**串起来
> （课堂按线框图改造 / 后台字体 / 教学素材预览 / **学生环境被 OOM 杀掉的事故** / 工作台与机构课包库存），
> 并记下三件「现在别做」与七条判断口径；⚠️ **它写的生产状态是当时的，别当现在用** ——
> 现在的看本文下面的「二、现在的状态（生产）」。
>
> 这两处交接里最值钱的两条，先记在这里：
