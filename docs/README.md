...
> 版本：2026-09-19（第二十轮 · 作品广场重做 + 导入 476 件 + 抓 aimagc 9 件网页作品 + 后台编辑/删除 + 分类表单 + 常见问题档位 + 机构手册重做）
> **⚠️ 新会话先读** `docs/operations/新对话交接-第二十轮-作品广场与导入-20260919.md` ——
> 那一份写清了**当前生产状态**（release **20260919T140543Z** / commit **e8f987c**、五入口 200、
> 广场 **205 件**、媒体 1.7GB）、**这一轮做了什么**（广场按参考站重做 / 导入 476 件 /
> nginx `/media/` + `.mov`→mp4 / 机构名随机化 / 分类只剩两个且后台可配 / 每页 12 件翻页 /
> 外链作品下架 / 图标换线性 SVG / 实心紫换紫粉渐变 / ⭐ **抓 aimagc.cn 那 9 件静态网页作品上广场** /
> ⭐ **后台作品编辑 + 软删/彻底删除两档 + 分类映射表单** / ⭐ **常见问题空档位隐藏 + 三个标签后台可排优先级** /
> ⭐ **机构手册按设计稿整页重做（图自托管、海报就位）** / ⭐ **删掉「灵动介绍」整页与其导航** / ⭐ **课时去掉三栏表单 + 框体没生成时能删 + 占位图换新插画**）、
> **不许破的口径**（第十五～十九轮 25+ 条 ＋ 本轮新增 **24** 条）、**结构与关键文件**、
> **下一步**（aimagc 若还有别处的作品需用户给清单、客户端那三件、老欠账），
> 以及**这一轮踩过的 36 个坑**（含怎么跑 p115 / p120 / p121 / p122 四道守卫）。
> 其中七条最值钱：⭐ **`.mov` 的容器品牌 `ftypqt` 浏览器播不了**（改 MIME/去 nosniff 都没用，
...
> 再在浏览器里**就地**把它改成 `clip` 滚一遍，即可实锤修法（这次就是这么定案的）。
>
