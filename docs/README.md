...
> 版本：2026-09-18（第十七轮 · 官网十一条口径收尾 + 网站浏览器守卫 p115）
> **⚠️ 新会话先读** `docs/operations/新对话交接-第十七轮-官网收尾与守卫-20260918.md` ——
> 那一份写清了**当前生产状态**、**这一轮做了什么（按主题，11 条口径）**、**不许破的口径**
> （第十五轮五条 ＋ 第十六轮六条 ＋ 本轮新长出的 8 条）、**官网现在的结构与关键文件**、
> **下一步（哪些是等你回话的、哪些是我建议优先做的）**，以及**这一轮踩过的 10 个坑**。
> 其中五条最值钱：**"本地全绿"≠"与生产对齐"**（我把首页兜底改反了方向，发布后打线上才发现）；
> **参考稿给的交互别自己"优化"掉**；**用户一条消息里连说几件事时，"第 N 条"未必对应那一页**；
> **两页同名时以用户截图里的 URL 为准**；**"偶发守卫红项"是一个类**（真根因是 `schema.js` import 时跑迁移、
> 多进程各跑一遍，正解是跨进程锁，不是再加 pragma）。
>
> 下一层仍然有效的那几份（**按需读，不必都读**）：
