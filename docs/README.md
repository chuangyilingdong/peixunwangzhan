...
> 版本：2026-09-17（第九轮 · **已上线**）　｜　**生产 release `20260917T035520Z` / commit `95a0b50`**
> 第九轮（本轮）修了两个 bug（**新会话先读** `docs/operations/新对话交接-网页搜索与秒进-20260917.md`）：
> ① **dsh 的网页搜索接到我们网关上**。查清后确认原设想是错的 —— 那个插件调的**不是搜索接口**，
> 而是 **Anthropic 协议的 `/messages`**（搜索是模型一跳里的服务端工具 `web_search_20250305`），
> 所以加的是一条 **Anthropic 透传**（`/api/gateway/v1/search/messages`）：学生环境拿运行时密钥打进来，
> 网关验签 → 过门禁 → 换成渠道真密钥转发 → 记账。**渠道真密钥绝不进学生环境**（学生读得到自己的
> 进程环境）。身份/门禁/记账与聊天那条路共用同一套实现。已端到端实测：真密钥 + 照插件源码发的请求
> → 200、10 条真实搜索结果、账本记下 tokens 与折算成本。
> ② **修「agent 读截图读到一半整轮失败」**（用户现场报的）：分发层对所有非 multipart 的 POST 一律
> `readJson(req, '2mb')`，**网关那条也不例外** —— 而 dsh 每轮重发全量历史，图片按**字节**很大、
> 按 **token** 很小（那一轮只有 25k tokens，请求体却是 3-4MB），于是整轮被 `PAYLOAD_TOO_LARGE` 掐掉。
> 现在 `/api/gateway/` 单独放宽到 24MB（普通接口仍守 2MB），并给历史图片加了**字节安全阀**
> （超了从最新往回留，更早的换成一句「需要时重读文件」——图本来就是 agent 自己读的）。
> 实测：3.26MB / 12 张真实截图的请求 → 200，模型真看懂了。
> ③ **彻底解决「历史没有尽头」**（用户点名的要求）：把 dsh 的**按路由图片预算**
> `maxRequestImageBytes` 设成 8MiB。机制是 —— **压缩救不了这件事**：它按 token 触发，
> 而图片在 dsh 被计价的投影里只是一个附件引用，字节的墙总是先到；正解是让 dsh 超预算时把
> **最老的**图从这一次请求里省略（历史不动、最新的一定看得见）。实测同一批 8 张截图：
> 默认 2.19MB 且随会话增长 → 设上限后**钉在 0.95MB**，请求体上界恒 ≤ ~8.5MB。
> ⚠️ 补丁层是**宿主文件**（`/opt/dsh-runtime/etc/dsh/student-runtime.cordis.yml`，手工装、
> 启动时读一次），改完必须重启学生环境。
> ⚠️ **「秒进」卡住的不是代码，是内存**（本轮实测：机器 1607MB、平台自己约 320MB、
> 一个学生环境 445MB → **同时只装得下 1–2 个**），所以**下一轮要先问用户要哪种形态**：
> 加内存（用户已表示自己会加）+ 开课预热全班，还是换掉 per-student 进程这个形态。
> 全量守卫 **116/116**（新增 p106 搜索透传、p107 客户端请求体预算；p97 扩了请求体上限与图片预算两组断言）。
