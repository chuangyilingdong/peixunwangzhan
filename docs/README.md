...
生产：release 20260918T090418Z / commit ef947be（服务 learning-platform-production @127.0.0.1:8789）
      ...
      ①**强刷不再闪旧内容**： 的启动骨架屏还是更早那版文案与配色，重做成品牌一致的加载态
      （黑底 + logo + 「正在进入课堂」+ 加载线，**不再写会过期的营销文案**）；首页的 
      还留着「给机构一套 / 能落地的青少年 AI 课 / 响应教育部…」，接口回来前会先渲染它 → 兜底对齐成 CMS
      当前内容，并且**接口回来前不渲染文案**（；视频与按钮不依赖 CMS，不会白屏）。
      产物里旧串已归零（「给机构一套」实测 0）
      ②**登录页不再有边框痕迹**：根因是  那层环境光斑被网格栏（overflow:hidden）
      ...
      右侧按钮组会跟到 logo 后面（还压住导航），给  补 margin-left:auto；手机端补在汉堡上
      ④**导航加玻璃底衬**（圆角 999 + 半透明白渐变 + 内高光 + 投影 + backdrop-blur），
      黑底首页白玻璃 / 浅底内页亮玻璃，同一结构换色；链接改成药丸高亮 + 当前项圆点
      ⚠️ 同批更新了守卫  的两条断言（口径变更）： 从旧文案
      「登录你的工作台」改查占位「请输入登录名」； 原来查的「预约演示」已不存在（等于失效），
