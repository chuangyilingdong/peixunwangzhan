...
> 版本：2026-09-20（第二十二轮 · 作品浏览 + 课堂口径）
> **⚠️ 新会话先读** `docs/operations/新对话交接-第二十二轮-作品浏览与课堂口径-20260920.md` ——
> 那一份写清了**当前生产状态**（release **20260920T080053Z** / commit **a756e9a**、五入口 200、
> 广场 **205 件**（其中"能在我们站直接跑的网页作品"9 件）、媒体 1.7GB），**这一段做了什么**（九件：
> ⭐ 广场查看层改「逻辑视口 + 等比缩放」，任何屏幕零滚动条 / ⭐ 学生端删掉「学习统计」（路由留重定向）/
> ⭐ 「我的作品」改成**真能打开**、并按内容切换的看（网页能玩 / 图片能一张张选 / 文档 / 画布）+ 写明「课包 › 课时」/
> ⭐ `client-context` 报出"这是哪节课"、并可**指定这次算哪节** / ⭐ 开课**复查占用**（补历史数据的洞）/
> ⭐ 删掉三个测试课包（学生作品保留、引用置空）/ ⭐ 官网删掉价格与「按课包开通」、课包详情删「适学年龄」「参考价格」/
> ⭐ 课时删掉「编程任务 / 素材说明」栏 + 修好那句"账号异常或已停用"的**假红字** / 客户端发送次数显示（另一个仓））、
> **不许破的口径**（**25–36 在这一份**）、**结构与关键文件**、
> **下一步**（等你拍板的 5 件 + 已知没做的 2 件）、以及**这一段踩过的坑 37–47**（加上前面共 47 个）+ 怎么跑守卫。
> 其中几条最值钱：⭐ **私有素材必须走 `data:`、且用 `api.fetchDataUrl`**（生产 CSP 是 `connect-src 'self'`，
> blob: 只在 `img-src` 里 —— 能当图片地址、**不能**再 `fetch` 它）；⭐ **`max-height` 会静默吃掉你对 `height` 的改动**；
> ⭐ **界面上的预检必须与"真正那一步"同一口径**（本轮那条"账号异常或已停用"就是界面在撒谎）；
> ⭐ **iframe 是刻意的 opaque origin**，所以"自适应"只能做成"里面按逻辑视口渲染好、外面整体缩放"；
> ⭐ **生产数据会违反文档自称的口径**（种子/演示脚本直写库、早期窄校验）——判断口径要查真实库；
> ⭐ **动生产数据**：先备份 → `PRAGMA foreign_keys=ON` → 一个事务 → 复查（否则级联不生效、会删出孤儿）。
>
> 上一轮（**官网收口 / 抓 aimagc 那 9 件网页作品上广场 / 画布微调**那一段读它）：
> `新对话交接-第二十一轮-官网收口与画布微调-20260919.md` —— **口径 7–24 与坑 12–36 在它和第二十轮那份里**
> （第二十二轮接着往下编号，所以「见口径⑰」「坑㉜」这类引用三份通用）。
