...
> ⚠️ **有一版已提交但还没上生产**：`159f7fc`（「勾选要授权的学员」按表格重做：修掉复选框飘在名字
> 远处、行高被撑爆的布局，并改成按剩余人次封顶）。2026-09-18 本机**出口 IP 变成了 `123.90.5.82`**，
> 而阿里云安全组只放行旧 IP → **SSH 22 直接超时**（`ping` 通、22 不通就是这个特征）。
> 恢复办法：在云控制台把这个新出口 IP 加进安全组，然后照 RUNBOOK 发布（脚本已在 `.tmp/deploy-002.sh`，
> 改 `EXPECT_COMMIT=159f7fc` 即可）。
> 生产本身没受影响：复核时 `/org/` 仍是 200，跑的还是 `20260917T115852Z`（即 `d0e3452`）。对外售价：对话 1 / 图片 1 / 视频 5 / 音乐 2 元每次（库里按**分**存：100 / 100 / 500 / 200）
