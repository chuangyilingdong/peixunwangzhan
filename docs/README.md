...
> 版本：2026-09-19（第二十轮 · 作品广场重做 + 导入 476 件 + 分类/分页/图标/配色收口）
> **⚠️ 新会话先读** `docs/operations/新对话交接-第二十轮-作品广场与导入-20260919.md` ——
> 那一份写清了**当前生产状态**（release **20260919T112428Z** / commit **7783380**、五入口 200、
> 广场 196 件、媒体 1.7GB）、**这一轮做了什么**（广场按参考站重做 / 导入 476 件 /
> nginx `/media/` + `.mov`→mp4 / 机构名随机化 / 分类只剩两个且后台可配 / 每页 12 件翻页 /
> 外链作品下架 / 图标换线性 SVG / 实心紫换紫粉渐变）、**不许破的口径**（第十五～十九轮 25+ 条 ＋
> 本轮新增 6 条）、**结构与关键文件**、**下一步**（aimagc.cn 静态作品可抓且有现成沙箱外壳、
> 后台作品编辑/删除与分类表单、全站配色是否统一、客户端那三件），以及**这一轮踩过的 11 个坑**。
> 其中三条最值钱：⭐ **`.mov` 的容器品牌 `ftypqt` 浏览器播不了**（改 MIME/去 nosniff 都没用，
> 要 `ffmpeg -c copy` 转 mp4）；⭐ **媒体不能复用 `/downloads/`**（那是 octet-stream + nosniff，
> 图片视频会被拒收）；⭐ **广场的作品要"四样齐全"才显示**（is_public / PUBLISHED / share_token /
> copyright_confirmed_at，缺一个不报错也不出现）。
>
