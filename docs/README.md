...
> 版本：2026-09-16（第三轮）　｜　**本地代码提交见 `git log -1`；生产跑的是 `9e7520c`**
> （release 20260916T101237Z），服务正常 —— 本轮改动**尚未部署**。
> 学生端 VibeCoding 正在迁到 dsh（DeepSeek Harness；**学生看到的品牌是「灵动ai」**）：
> **学生创作环境这条路已在真机上整体跑通**（同机独立用户隔离、公网入口、界面、模型走我们网关落账）；
> **「产物回传」的后端也做完了**（学生工作区里的作品能列出来、取回来、按现有作品链路落库，
> 网页作品端到端跑通，含作品里的本地图片；收环境前会先把作品留存一份）。
> 还差三件：① **学生页面上的按钮**（`/api/student/runtime/*` 目前没有前端在调）
> ② **二进制产物（PPT/Word/Excel 原文件）的收纳与广场展示** —— **要先定口径**（见交接第四节）
> ③ 平台侧部署 + 宿主 sudoers 放行 `collect-student-user.sh`。
> 见 `docs/operations/VibeCoding改用dsh-方案-20260916.md`，
> 以及**最新交接** `docs/operations/新对话交接-dsh迁移-20260916-第三轮.md`
> （它纠正了上一轮两处说法：二进制产物走不了现有 `/submit` 那套规则；现有作品链路只认同层扁平文件名）。
> 整体 MVP / 1–7 项仍不能标为完成。**
