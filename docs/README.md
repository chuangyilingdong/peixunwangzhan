...
> 版本：2026-09-19（第十八轮 · 客户端登录门 + 常见问题按端三档 + 发送次数口径）
> **⚠️ 新会话先读** `docs/operations/新对话交接-第十八轮-客户端登录门与服务端次数口径-20260919.md` ——
> 那一份写清了**当前生产状态**、**这一轮做了什么（按主题：官网 FAQ 两轮改造 / 桌面客户端 / 服务端次数口径 /
> 官网下载入口）**、**不许破的口径**（第十五～十七轮共 19 条 ＋ 本轮新增 6 条）、**结构与关键文件**、
> **下一步（等你回话的 / 我建议优先的）**，以及**这一轮踩过的 13 个坑**。
> 其中三条最值钱：⭐ **宿主（dsh）在登录门之前就起来了** —— 客户端登录后拿到的网关密钥写
> `process.env` / 写凭据文件都到不了 dsh，每一轮都「API 密钥无效」（**客户端现在卡在这，安装包没对外开放**）；
> **补丁层要挂桌面宿主的 `patchFiles`，不能写 profile 的 `cordis.patch.yml`**（空数组不是 undefined，
> 那个文件永远不会被读）；**改内容形状前先 grep 该 key 的所有读方**（`/org` 那块静默退回写死兜底）。
>
